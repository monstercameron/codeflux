...
- [x] `AUDIT-005a REVIEW DATA` Implement the project and thread deletion path `DefaultDeletionPolicy` declares, or delete the unused policy type: `DeletionPolicy`, `DeletionModeTombstoneThenPurge`, and `RequireOrphanCheck` have no executor and no consumer, so the declared user-data deletion semantics are unimplemented while reading as settled.
  - **Resolved by the second alternative: deleted.** A real project and thread purge path carries a migration, an authority model, and irreversible effects on user data. That is a feature with its own governing task, not a reconciliation of the completion ledger.
  - The declarations lived in `internal/storage/maintenance.go`, not a `deletion*.go` file. `DeletionMode`, both mode constants, `DeletionPolicy`, and `DefaultDeletionPolicy` are gone. `DeletionModeHardDeleteLineage` went with them: it named the one mode that is genuinely executed, by `DeleteMemoryArtifact`, which reaches that behaviour without consulting a policy struct.
  - `TestMaintenancePoliciesAreExplicit` asserted the four deletion fields against the declaration's own literal values, so **nothing executed them and it could not fail for any reason that mattered** — the schema refuses the deletions the policy called `tombstone-then-purge`, and the test passed anyway. Narrowed to the retention policy, with the reason recorded at the test.
  - `internal/storage/deletion_policy_declared_unconsumed_test.go` guards the direction of the repair rather than the deletion: the compiler already enforces that the symbols are gone, but not that they stay gone until an executor exists. It scans production source only — test prose deliberately names them so a reader finding the gap can find out why. Follows `cmd/codeflux-dev/vector_branch_test.go` from `AUDIT-026`. **Proven to discriminate:** a temporary production reference was detected and named, then removed.
  - **The capability the repository does not have, stated plainly:** there is no project or thread deletion or purge path at all, and the schema actively refuses such deletions. Filed as `AUDIT-005b`.
  - Verified: `go build ./...`, `gofmt`, and `go test ./internal/storage/ -run 'TestAUDIT005|TestMaintenancePolicies'` green.
- [ ] `AUDIT-005b DATA` Implement project and thread deletion as its own feature: a migration declaring the lineage behaviour, an authority model for an irreversible user-data operation, a tombstone-then-purge executor, and tests against a real SQLite database. Today deleting a project or graph with descendants is refused atomically because no migration declares `ON DELETE` and the DSN enforces foreign keys, so a user cannot delete their own project through any supported path. `AUDIT-005a` removed the policy type that described this without performing it; this ticket is the work that description was standing in for.
...
- [ ] `AUDIT-010a REVIEW E2E` Add the mounted browser check for the expandable context card now that `context_manifests` is populated by real runs, closing the presentation half of `M08-045` and `M08-G03`.
  - **Scope correction: this is not "add the missing browser check".** A search of all of `web/` for `ContextCard`/`ContextManifest` finds **nothing**. The entire presentation layer is absent — the RPC exposing `context_manifests` and the GWC component itself both need building before a mounted check has anything to check. That spans proto, transport, a new frontend surface and then the browser test.
...
- [ ] `AUDIT-011a REVIEW` Wire `InstructionApprovalResolverFor` into the production context-selection path and give first-use approval a user-facing grant surface, so a repository instruction can be approved rather than only refused.
  - **This ticket's premise is stale and is corrected here: the wiring already existed.** `execution.repositories.InstructionApprovalResolverFor(...)` has been called from `selectAgentContext` (`agent_execution.go:511-516`) since commit `92f1eed6`, before this session. What was missing was **proof** — no test drove a genuinely granted approval through the real function.
  - `TestAUDIT011a_SelectAgentContextHonorsARealGrantedApproval` calls the actual unexported production `selectAgentContext` with the actual production resolver construction, a real migrated SQLite `Repositories` and a real resolved Git revision: refused before a grant exists, admitted with the exact approved bytes after a real `GrantRepositoryInstructionApproval`. Passes at `-count=3`.
  - **Building that proof surfaced a separate pre-existing defect, filed as `AUDIT-011b`:** every engine-fixture-based full run in this package *always* degrades context selection with "repository map revision is stale", because `EnsureLocalBootstrap` binds `git_identity` once and nothing ever resynchronises it to the HEAD a task's worktree is actually created from. `resolveWorktree` resolves a real HEAD for the worktree and never writes it back. **The success branch this ticket is about is therefore unreachable through a full `StartTask` run in that fixture family.** The test sidesteps it explicitly rather than routing around it silently.
  - **The grant surface stays open and was deliberately not attempted.** Every caller of `GrantRepositoryInstructionApproval` is still a test. Building the creation path spans transport, proto and frontend, and a *creation* endpoint for admitting repository text into a run is precisely the shape `MEM-009` was left uncalled to avoid.
- [ ] `AUDIT-011b BLOCKER` Resynchronise `git_identity` to the revision a task's worktree is actually created from. `EnsureLocalBootstrap` binds it once and nothing updates it; `task_run_launcher.go`'s `resolveWorktree` resolves a real HEAD for the worktree and never writes it back. The consequence is not cosmetic: **every engine-fixture-based full run degrades context selection with "repository map revision is stale"**, so `selectAgentContext`'s success branch — the one `AUDIT-011a` exists to exercise — is unreachable through a full `StartTask` run in that fixture family. Found while building `AUDIT-011a`'s proof, which sidesteps it explicitly rather than routing around it silently.
...
- [ ] `AUDIT-016a REVIEW BLOCKER` Implement `WorkspaceService.OpenWorkspace` against a durable repository approval so the "repository open" journey completes over the API rather than only in-process via `EnsureLocalBootstrap`. `AUDIT-011`'s `repository_instruction_approvals` is the shape to follow: grant bound to project and path, immutable, consumable, revocable.
  - **Blocked, and deliberately not half-built.** `WorkspaceService.OpenWorkspace` is a *deliberate, tested refusal* — `TestOpeningAWorkspaceIsRefusedRatherThanFaked` pins it — and no `RepositoryOpenApproval`-shaped mechanism exists anywhere. Completing it means building a new grant-creation surface spanning migration, repository, transport RPC, proto and probably frontend.
  - Building only the storage half would produce a table with no caller, which is exactly the "found implemented, zero callers" finding this session has now hit six times. Reported blocked rather than adding a seventh.
- [x] `AUDIT-017 REVIEW SECURITY` Reconcile `M11-033`: expose a validated, explicitly authorized user container command through config, application options, task launch, and supervisor wiring, then test container and native paths.
  - **Closed, and the gap was configuration, not implementation.** `worker.buildWorkerCommand` and `Supervisor.StartWorker` already implemented and propagated a container-wrapped launch, with their own tests. **Nothing between config and the coordinator ever populated it** — `ApplicationOptions` had no such field and `TaskRunLauncher.Launch` never set `StartWorker.ContainerCommand` — so **every task ran natively regardless of intent, because there was nowhere to say otherwise.**
  - `ContainerCommand []string` is threaded from `ApplicationOptions` through a new `validateContainerCommand` in `buildRunLauncher` into `NewTaskRunLauncher` and `Launch`'s `StartWorker`.
  - **The discriminating test proves execution, not assignment:** `TestAUDIT017_AConfiguredContainerCommandWrapsTheRealWorkerLaunch` drives a real `StartApplication` → real `StartTask` with the option pointed at a compiled marker program that writes a file naming its own argv. The file appearing proves the coordinator executed the configured wrapper as the worker's process, rather than proving a struct field was set. Verified independently: PASS, 2.77s.
  - **Left undone:** no CLI flag, since `cmd/codeflux-dev` was mid-edit by another lane. `ApplicationOptions.ContainerCommand` is the configuration surface, consistent with how `WorkerExecutable` already works.
...
- [x] `AUDIT-018a REVIEW` Signal the dispatch pump when a pause is lifted and when an approval is granted, so unblocked work starts on the transition rather than on the fallback tick, and test fairness across pause, approval, and restart.
  - Three transition sites signal, each **after** its durable commit, never before: `TaskControlService.ResumeTask` after `ResumeControlledTask` returns, `TaskLifecycleAdapter.ApproveTaskPlan` after `advanceToReady` returns, and `WorkerRuntime.RegisterRecoveryStart` after registration, with `runtime.mu` released first. Signalling before the commit would let a drain observe the old blocked state and reintroduce the wait as an intermittent bug rather than a constant one.
  - All three reuse the existing non-blocking send against a buffer of one, so a transition is never delayed by the pump.
  - **The tests structurally exclude the fallback tick** rather than racing it: `startDispatchPump` gained an optional interval and the tests set it to one hour against a ~500ms observation window, so a start inside that window cannot have come from the tick. Each also re-reads durable state from inside the injected callback, proving the signal fires after the commit rather than merely in the right call order. `-count=5`: 15/15, no flakes.
  - **The wiring is the part a behavioural test cannot reach, and it is why `AUDIT-018` shipped incomplete.** The pump tests inject a callback directly, so with the application-side wiring absent every one of them still passes while production falls back to the tick. `internal/coordinator/dispatch_wiring_test.go` reads the three construction sites in `application.go` and fails if any is dropped. Reading a construction site is weaker than observing a signal; that limitation is recorded at the test rather than implied.
  - `newApplicationTaskControls` takes the notifier as a parameter; nil is accepted and means nothing is signalled, which is right for a control surface built without a pump.
  - Verified: `go build ./...`, `gofmt`, `go vet`, `staticcheck`, and `go test ./internal/coordinator/ -run 'TestAUDIT018|TestDispatchPump|TestWorkerRuntime|TestApplication'` all green.
- [ ] `AUDIT-019 REVIEW BLOCKER` Reconcile `M12-051`, `M12-052`, and `M12-G04`: wire provider recovery into exhausted execution, expose retry/resume/explicit-switch commands, persist exact from/to authority, and prove no adapter fallback silently changes provider or model.
  - **Re-verified unchanged and correctly still open.** `NewProviderRecoveryService` and `providers.ValidateProviderSwitch` still have zero non-test callers. The blocker is a **plan decision** rather than an implementation one — whether a configured ladder rung constitutes explicit user authority for a provider switch — and that is not a subagent's call to make.
...
- [x] `AUDIT-020a` Acknowledge a run's own durable state transition from `starting` to `running`. **No production code anywhere performed it** — confirmed by full symbol search: `domain.RunStateRunning` had zero non-test references outside `domain/states.go`, so every run's row stayed `starting` for its whole life while the task moved through its own states independently.
  - **Closed, and the survey found a live product defect rather than a bookkeeping gap.** `RequestTaskPause` (`internal/storage/task_control_repository.go:475`) gates on `runs.state IN ('running','validating')`. Since a run's row sat at `starting` for its entire executing lifetime, **pause was unusable for the whole time a run was actually doing work** — only the brief `validating` review window ever admitted it. `ResumeControlledTask` confirms the shape: it puts the run back to `starting` on resume, and nothing ever advanced it again.
  - **Unaffected, checked rather than assumed:** `CancelControlledTask` gates on a blocklist rather than an allowlist; recovery-candidate discovery treats `starting` and `running` identically; `inspect` displays the raw value without gating.
  - **Fix:** `transitionRunStartingToRunning` in `agent_execution_repository.go`, called unconditionally at the top of `BindRunPlan`'s transaction — the earliest durable once-per-attempt fact that real work has begun, since the agent loop calls it before any tool call, and it re-fires correctly after a pause/resume cycle. `TransitionRunToValidation` keeps the same helper only as a documented fallback for a run reaching validation without ever binding a plan.
  - Two existing tests that forced `starting`→`validating` via raw SQL needed `IN ('starting','running')` widening, since the row may legitimately already be `running`. `domain/states.go` was not touched — the transition was already declared valid.
  ...
  - **Attempted 2026-08-02 and backed out. The wiring hangs real runs, and that is the finding.** Measured decisively by removing only the `AUDIT-020` changes and re-running the same test: `TestTheEngineCarriesARequirementThroughToRealWorkOnDisk` takes **3.8s and passes** without them, and **times out past 480s** with them. Everything else in the tree was unchanged across both runs, so the attribution is not in doubt even though the mechanism is.
  - The attempt is preserved as a patch under `.artifacts/tmp/audit020-backout/` — **ephemeral, and will not survive a clean**. Redoing it should start from this entry rather than that file.
  - **Three real discoveries from the attempt, worth keeping regardless:**
    - `LoopDependencies.PlanSteps` was wired to a **no-op** (`agentNoDurableJournal{}`), so no plan-step transition was ever durable — and `PrepareCompletion`'s evidence check requires one for every declared step. That alone would have blocked completion even with the service constructed.
    - `AgentExecutionPersistence` in `agent_execution_persistence.go` is a **complete, tested, never-called** implementation of `PlanStepStore`. The right fix uses it rather than adding a fourth parallel implementation.
    - Nothing durably produced the `runs.state` → `validating` transition; `TransitionTask` only touches `tasks`, and none of the other `Transition*` functions covers it. A new storage function is genuinely needed.
  - **Prime suspect for the hang, not yet confirmed:** the attempt ran a final `go test ./...` through the tool executor at the end of every run. In a worktree of any size that is unbounded work on a path that previously did none. Diagnose that before re-attempting; a completion path that cannot finish is worse than one that never starts, because the run now hangs instead of stopping honestly in `running`.
  - A second unchecked risk: `MEM-001a` made episode closure depend on a task reaching `awaiting-review`. Whoever re-attempts this must check the two agree — an episode that never closes, or closes twice, is the seam between those changes.
  - The `CODEFLUX_AUDIT020=1` gate stays until a re-attempt can show the run reaching `awaiting-review`. It was briefly removed and restored, correctly: an ungated hanging test in the shared suite is worse than a gated reproduction.
...
- [ ] `AUDIT-027 REVIEW BLOCKER TEST` Reconcile `M22-036` through `M22-050` and `M22-G03`: connect fault injection to the real worker, coordinator, browser, provider, command, worktree, and SQLite boundaries; the current generic `RunWithFault` ledger never enters those production paths.
  - **Partially closed: the SQLite boundary, one of the six named.** `FaultInjector` did not exist in `internal/storage` at all. `transaction.go` now carries the interface, `Database.SetFaultInjector`, and two points consulted at genuine single choke-points every mutation passes through — `FaultPointDatabaseBusyTimeout` inside `RunInTransaction` before `BeginTx`, and `FaultPointDiskFullOnEventAppend` inside `appendTaskEventTransaction`, which six production sites call, before the `INSERT`.
  - The test arms the real `testfixtures` vocabulary against the real `RunInTransaction` and the real `AppendTaskEvent` and asserts the arm is **consumed**, not merely accepted, plus unarmed and nil-injector no-op cases and a vocabulary-drift guard. Verified independently: PASS; full storage package PASS at 204s.
  - **Five of six remain:** coordinator event commit, worker lifecycle, provider streaming, worktree edit and browser transport, living in `internal/coordinator`, `internal/gitwork` and `internal/providers`, several under active edit by other lanes.
...
- [x] `AUDIT-029a REVIEW` Remove or implement `ApplicationOptions.SimulateExecution`: it is documented at length, set from `cmd/codeflux start --simulate`, and consumed nowhere, so the flag reports a capability the coordinator does not have. Same class as the unused `DeletionPolicy` in `AUDIT-005a`.
  - **Removed.** Two sites existed and neither read the value: the field and its eight-line doc comment in `internal/coordinator/application.go`, and the flag parsing, wiring, and "execution is SIMULATED" banner in `cmd/codeflux/start.go`. `commandRegistry()` never documented `--simulate`, so the help text needed no change.
  - `cmd/codeflux/simulate_flag_removed_test.go` pins both halves: `parseStartArguments(["--simulate"])` must now error as an unknown flag, and a walk of `internal/` and `cmd/` fails if `SimulateExecution` or `--simulate` reappears in production source.
  - **Behaviour change worth stating:** `codeflux start --simulate` previously printed a banner claiming execution was simulated and then executed normally. It now exits with a usage error. Refusing a flag is a smaller lie than honouring it falsely.
  - Verified: `go build ./...`, `gofmt`, `go vet`, `go test ./cmd/codeflux/ -count=1`, and `staticcheck` over the three affected packages all green.
  - Follow-up if a real mode is ever wanted, not filed as a ticket because nothing asks for it: a scripted-execution executor producing `SIMULATED`-stamped events and files, wired behind `ApplicationOptions`, with a coordinator test proving a simulated run completes without a provider.
...
- [x] `PIPE-004a` Resolve every declared stage performer against a real function, because `ValidateChecks` only required the name to be non-empty and free of spaces — so a name resolving to nothing passed, and **six of the thirty-seven did**.
  - Four named functions that have never existed anywhere in this repository: `checkDecompositionCoverage`, `synthesiseAtomCases`, `checkAtomPropertyTests`, `checkAssembly`. Two named functions deleted as unreachable under `REPO-018` while the table kept pointing at them: `documentAtoms`, `measureCoverage`.
  - The failure is quiet and specific: the binding still reads as authoritative, so a reader chasing a satisfied ledger row is sent to a function that is not there and concludes the *table* is stale rather than that the *stage* is unperformed. `PIPE-017` had already corrected two such names by hand, which is the evidence that hand-checking does not hold.
  - Corrected to what really decides each stage: `decomposition-coverage` and `assembly` are decided inline by `AgentExecution.Run` through `ledger.require`; `atom-case-synthesis` is `checkCaseCoverage`; `atom-property-tests` is `checkPropertyTests`; `atom-documentation` is `checkAtomDocumentation`; `path-coverage` is `checkFunctionCoverage`.
  - `internal/coordinator/stage_performers_test.go` parses the package's own declarations and resolves every `Performer`, handling both the plain and `Receiver.Method` forms. It refuses to run against zero parsed declarations, so it cannot pass vacuously. **Proven to discriminate:** restoring `measureCoverage` makes it fail naming that entry and its stage.
...
- [x] `PIPE-006a UX` Show recorded stage duration in the ledger view now that `PipelineStageRecord` carries a real span and `Duration()`.
  - **Done, with the browser evidence honestly short.** `web/frontend/pipelineledger` renders the stage table and the skip headline, mounted into the already-live review panel rather than into `main.go` or `shell/**`, which another lane held — a genuinely reachable surface instead of a contended one.
  - **Unmeasured is distinguished from instant on three non-colour channels**, as `AGENTS.md` requires: different accessible text ("Not measured" versus a formatted duration), a different status glyph, and a different DOM shape (badge versus plain span). The decoder reads `ElapsedMeasured` first and **discards a wire `Elapsed` value when the flag is false**, so even a malformed upstream response cannot leak a false duration. A test pins the case that matters — a *measured* stage that genuinely took `0s` renders its real zero and is never conflated with the untracked one.
  - **A real wire gap, spelled out rather than papered over:** `PipelineStageView` carries state but not `PIPE-042`'s classification, which lives only in the stage-39 row's evidence payload. Rather than collapse to a single ratio, the client recomputes the classification from `internal/pipeline`'s own static tables — precedented by `evidencereport` importing `internal/evidence`. What it still cannot show is the run's own free-text reason, which stays server-side.
  - **That recomputation immediately produced the defect this repository keeps finding, and it was caught and removed.** The two copies of the rule disagreed on day one: the coordinator classified a stage with no declared check as `unimplemented`, and the browser's copy folded that case in with may-not-satisfy, answering `no-check-by-design` — or `principled-decline` if the stage appeared in `Section33Resolved`. So the same ledger classified differently depending on who read it, **in the direction that flatters**: a stage the flow gained without a performer would have been presented as a deliberate design decision. `pipeline.ClassifyDecline` is now the single implementation both call, with the unbound case pinned by test.
  - **Browser evidence is partial and said so.** The shell was driven live and screenshots opened — it boots and renders correctly with the change compiled in. But the review panel could not be opened: its button is disabled with "There is no work to review yet", because a fresh database has no completed task. So the populated ledger, the skip headline and the breakdown are covered only by native projection tests, which is weaker than a viewed screenshot. That is `REPO-041` and `REPO-044` again in a third guise — no supported path produces a run to look at.
  - `test-browser` did not complete and no pass/fail is claimed from it.
- [x] `PIPE-044a` Populate `timelineview.Props.PipelineSkipSummary` so the run's final message actually carries the unverified caveat. The field, the nil-safe rendering, and `RunCompletionCaveat` all exist and are tested; nothing sets the field. `timelineview.Props` is constructed only in `web/frontend/shell/shell.go`, which was held by another lane, and the addition is small and precise: call `useMountedPipelineLedger` once per render and set the pointer before constructing each card's `Props`. Until then `PIPE-044`'s ledger half is visible and its timeline half is not.
  - **Closed, with browser evidence that was looked at rather than merely captured.** `PipelineSkipSummary` is threaded through `shell.TimelineControlProps` onto every card's `Props`; `useMountedReview` now returns the `SkipSummary` it already computed internally and `main.go` sets the pointer from it, **reusing the existing fetch rather than adding a second one**.
  - The end-to-end test boots a real `coordinator.Application` with `FrontendAssets`, drives one real requirement to `awaiting-review`, and drives headless Chromium to `/tasks`. The caveat rendered `"28% of stages established nothing — 13 established, 11 declined, 0 not implemented"`, cross-checked against a direct database read of that attempt's 41 `pipeline_stage_records` rows excluding the skip-audit stage itself, matching `ComputeSkipSummary`'s own exclusion. The numbers agree exactly. Opt-in via `CODEFLUX_PIPE044A_BROWSER=1`, skipped by default.
  - **A real bug found en route and only partly fixed, reported as such.** Instrumenting `useMountedPipelineLedger` showed the fetch could settle permanently on the "no task selected" load's resolved error the instant a thread was selected — `fetch.UseResource`'s dependency comparison missed the transition, matching this project's own "deps are a hint, not a guarantee" caveat. A `UseRef`-guarded explicit `Reload()` measurably improved the pass rate but **did not make it deterministic**: it passed the large majority of runs with correct data every time it passed, and occasionally failed to converge within 90s on a checkout six lanes were hammering. Not claimed as reliable.
  - **Blocked, and the reason is bigger than this ticket: there is no ledger view, and no wire to build one on.** An investigation across all 33 `web/frontend` packages found nothing binding to `storage.PipelineStageRecord`, `pipeline.Number`, or `pipeline.Flow`. `ListPipelineStages` is called only from `internal/coordinator` and never from `internal/transport`. No RPC and no session-event kind carries stage rows, so the 38-stage ledger — the repository's central evidence artifact — **cannot be seen by a user at all**.
  - The nearest surface, `executionview.ExecutionTimeline`, is built in `web/client/execution_projection.go` from generic `events.SessionEvent` values that carry no stage number and no start or finish time. Rendering duration there would have shown ordinary session events dressed as pipeline stages, which is why it was not done.
  - `assembleEvidence` does count satisfied, failed and not-examined stages, but writes that count into the ledger's own `evidence-bundle` row and nowhere else — not into `internal/evidence.Report`, which is populated from entirely separate tables.
  - **To unblock:** expose `PipelineStageRecord` per task and attempt — stage name, state, and either `StartedAt`/`FinishedAt` or a duration plus a `Measured` flag — as a transport RPC over `ListPipelineStages`, or as a durable session-event kind published from `pipelineLedger.record`. Filed as `PIPE-006b`. Once it exists this ticket is small.
- [x] `PIPE-006b BLOCKER DATA` Expose the pipeline stage ledger over the wire. Nothing outside `internal/coordinator` can read `pipeline_stage_records`: there is no RPC and no session-event kind, so the flow's per-stage verdicts, durations, and skip counts are invisible to any interface. This blocks `PIPE-006a` and `PIPE-044`, and it means the evidence the whole Pipeline Refinement milestone produces currently has no reader.
  - **Done as an RPC, not a session event, and the reasoning matters.** The ledger's rows are immutable and fully written before any interface could announce them, so there is nothing to stream that is not already durable. They are addressed by task and attempt rather than delivered in one session's live order, and `SessionProjectionSnapshot` is deliberately correctness-bearing base state, not a stage-by-stage audit trail. An event would have bought live per-stage animation at the cost of threading a payload through every write site and multiplying session-event volume by up to 38 per attempt for data already in SQL. `PipelineStageService.ListPipelineStages` sits beside `GetDiffSummary` and `GetEvidenceReport`, which is the company it belongs in.
  - **`StartedAt`/`FinishedAt` alone cannot answer the honesty question.** An untracked start and a genuinely instant stage store identical equal timestamps. Migration `000036` adds an explicit `elapsed_measured` column, decided once at write time in `RecordPipelineStageResult` — the only place that knows *why* the two are equal — and carried to the wire beside `elapsed`. The handler leaves `elapsed` **unset** rather than an explicit zero when unmeasured, so a client reading only that field cannot misread it.
  - **Proven to discriminate twice.** Sabotaging the storage write to infer measured-ness from timestamp inequality made the re-read assertion fail; sabotaging the handler to always set `elapsed` made the wire test fail. Both reverted and re-verified.
  - Bounded at `LIMIT 128` as defence in depth — the schema's own `stage_number BETWEEN 1 AND 64` CHECK and the uniqueness constraint already cap an attempt at 64 rows — with a test proving the bound does not clip a real 38-stage flow.
  - Named `elapsed`, not `duration`, matching `TaskView.elapsed`, and documented at the proto as wall clock from the previous stage's record rather than the check's own processing time.
  - **Limitation:** `attempt=0` resolves to attempt 1, matching `NextPipelineAttempt`'s convention. There is no "latest attempt" concept anywhere in the product, so this surface does not invent one.
...
- [x] `PIPE-019 BLOCKER` Require at least one executable acceptance example at the instructions stage and remove the declared-absence escape, so no run can satisfy the flow against a request nothing external checks.
  - **Attempted and reverted.** The gate itself is straightforward — record the instructions stage failed when no example of either form is present — and `PIPE-118` removed the reason it could not be required. What is not straightforward is the blast radius.
  - Nine test files drive real runs, one of them the concurrently-edited 5,000-line `engine_produces_program_test.go` with ~50 table cases. None supplies an acceptance example, so every one records a failed stage.
  - Making the fixture helper append a default example does **not** work, and finding out why cost several cycles: a requirement carrying an `<<<ACCEPTANCE` block on its own line ends in a newline, `storage.AnalyzeTaskRequirement` refuses an untrimmed body, and the refusal is **silent** — the ambiguity block is skipped, the plan store then rejects the requirement, and the run dies several stages later reporting a constraint rather than a whitespace problem.
  - Trimming `requirementOf` at the source looked like the right fix and **broke every run**: `scope.requirement` is evidently used somewhere as an exact key or digest, so a trimmed body no longer matched. Reverted. That is a real latent defect and is filed as `PIPE-019a`.
  - What remains for this item: migrate the nine fixture files to state examples, coordinating with whoever owns `engine_produces_program_test.go`, then flip the stage to failed. `pipeline.ClassesWithoutAnImplementedForm()` is empty now, so the gate is honest to turn on once the fixtures are ready.
- [ ] `PIPE-019a` Find why `scope.requirement` cannot be trimmed. Trimming it in `requirementOf` makes every run stop after roughly four stages, which means the raw body is being used as an exact key or digest somewhere downstream. Separately, `AnalyzeTaskRequirement` refusing an untrimmed body is silent: the ambiguity block is skipped and the failure surfaces much later as a plan-store constraint error. Both should be fixed before `PIPE-019` requires requirements that carry a multi-line acceptance block.
  - **Diagnosed by reproduction. Not fixed — the fix is in `internal/storage`, which the diagnosing lane did not own.** The guess in this ticket's own wording was wrong: nothing uses the raw body as a digest. There are **two independent computations** of `AnalyzeTaskRequirement` over the requirement, and trimming one does not help.
  - `storage.AnalyzeTaskRequirement` refuses any body where `strings.TrimSpace(body) != body`. `agent_execution.go`'s ambiguity check calls it and **swallows the error**, so clarification silently records not-implemented. `recordDurablePlan` also calls it and propagates, which leaves `plan.Revision` at 0, and every stage needing a bound plan never runs — the observed stall.
  - The part that defeats a source-side trim: `storage.RecordTaskRequirement` **does not trust the caller's analysis**. It re-reads the message body from SQL and recomputes the analysis on that raw value, requiring a byte-identical match. So trimming `requirementOf` fixes the first two computations and the third still refuses, just with a generic constraint error instead of a clear one. That is why the earlier attempt "broke every run" — it never could have worked.
  - Two permanent tests in `internal/coordinator/pipe019a_repro_test.go` pin both halves: a full run with a trailing newline stalls silently, and `RecordTaskRequirement` refuses a hand-trimmed analysis against an untrimmed stored message with `ErrConstraint`. The trim was applied, re-tested, confirmed insufficient, and reverted.
  - **To close:** normalise the requirement text once, either inside `AnalyzeTaskRequirement` before it validates, or where the message body is first persisted. Both are in `internal/storage`.
  - **Closed by normalising inside `AnalyzeTaskRequirement`**, which now trims before validating instead of refusing. That is the one function all three computations already call, so normalising there — and only there — makes disagreement structurally impossible without inventing a fourth definition of "the requirement text".
  - **Persist-time normalisation was considered and rejected on principle:** the stored message is the user's own content, and a fix for an internal analysis disagreement has no business rewriting what a person typed. Normalising the derived analysis leaves the message intact.
...
- [x] `PIPE-020` Add the acceptance-oracle stage: every example runs and fails against an empty program, proving the example discriminates before anything is built on it.
  - `StageAcceptanceOracle` added as number 38, **appended rather than renumbered** because `PIPE-045` owns renumbering. Its performer is `AgentExecution.checkAcceptanceOracle`, which reuses `checkAcceptance` against the pristine worktree before the attempt loop and inverts the meaning: an example already true before any work **fails** the oracle, and one that genuinely fails satisfies it.
  - Wired into `Run` immediately after `StageInstructions`, so its number and its position in the dependency graph diverge sharply — it is unblocked second while sitting last in the list. `PIPE-058`'s requirements table expresses that: it requires `StageInstructions` alone.
  - **Proven to discriminate:** inverting the `outcome.Held` branch makes `TestPIPE020_AnExampleAlreadyTrueBeforeAnyWorkFailsTheOracle` fail, and reverting restores green.
- [x] `PIPE-020a BLOCKER TEST` Give the escalation ladder test acceptance examples, or `PIPE-019` breaks it. `engine_produces_program_test.go` supplies no acceptance example anywhere, and its `reportFlow` helper fails the test on any failed stage row — so now that `instructions` requires an example, the next real run of the 251-rung ladder fails **every rung** at the first stage. It is gated behind `CODEFLUX_LADDER` and calls a paid model, so this is currently invisible and will surface as a total failure the first time somebody spends money on it. Nobody has run it since `PIPE-019` landed, and nobody should until this is fixed.
  - **The premise was already stale when this ticket was written, and the alarm was mine to have checked.** Commit `49061c4` had expanded the ladder to 251 rungs *the same day* and added `(generatedProgram).acceptanceBlock()`, appended to every rung at `carryOut(t, program.requirement+program.acceptanceBlock(), ...)`. All 251 rungs carry a non-empty `expected:`, so a real `<<<ACCEPTANCE … >>>` block renders for every one. There was no landmine.
  - Verified rather than assumed, without ever running the paid path. `internal/coordinator/pipe020a_ladder_acceptance_test.go` calls the **real** `parseAcceptanceExamples`, `parseNamedTestExamples` and `checkAcceptanceOracle` — not reimplementations — over all 251 rungs: each satisfies the instructions-stage predicate exactly as `agent_execution.go` computes it, each parses back to the `expected` the rung declares, and each **discriminates** against a bare git-initialised module with nothing produced. A third test is the control: a worktree pre-populated with a program that already satisfies its example makes the oracle report `broke`, so "held" is not vacuous.
  - **Independent pre-existing defect found by the round-trip assertion:** rung 25's `expected` began with a literal space (`" a\n-b\n+x\n c"`). Both `normalizeProgramOutput` and `parseOneExample` call `strings.TrimSpace` on the whole joined text, eating that first byte — so rung 25's own `got != program.expected` check **could never pass, even for a perfectly correct program**. Repaired by reordering the diff so the first differing line is a deletion, keeping the rung's existing deletions-before-insertions convention.
  - **Unverified until someone deliberately runs the paid ladder:** whether a real model shown this requirement text goes on to satisfy `StageEndToEndTests`, whether the other ~36 stages `reportFlow` asserts hold against real model output, and whether rung 25's new expected transcript is one a correct LCS diff would actually produce. Offline testing shows the string is self-consistent through parsing and normalisation; it cannot show it is achievable.
  - `CODEFLUX_LADDER` was never set and `run-live` never invoked; `TestTheEngineProducesProgramsThatBuildAndRun` still skips cleanly with its cost warning.
...
- [ ] `PIPE-042` Add skip-audit challenging every skipped and not-implemented stage and reporting the ratio as a headline figure in the run summary, not only in the ledger table.
  - **Implemented as stage 39, appended not renumbered; one line of wiring outstanding.** `internal/coordinator/skip_audit.go` holds the classification and computation, `checkSkipAudit` is its performer, and entries exist in `Checks`, `requirements.go` and `phases.go`.
  - **The classification is the substance, and it reuses `checks.go`'s existing vocabulary rather than inventing a second one.** A stage that can never satisfy and is named in `Section33Resolved` is a **principled decline** carrying its governing ticket; one that can never satisfy and is not named — `human-acceptance`, `deliver` — is **no-check-by-design**; a stage that *could* satisfy and recorded skipped is a principled decline made by a check that ran and examined this run's own facts; the same stage recording not-implemented is **unimplemented**. Satisfied, failed and blocked are counted but not classified: a failed check ran, and a blocked stage was never given a fair chance, so neither is a decline.
  - The classification is carried per entry rather than collapsed into the ratio, so a reader sees the headline **and** which declines were honest. That was the trap worth avoiding — a figure lumping "correctly declined" together with "never built" makes honest runs look evasive and teaches people to stop declining honestly.
  - Computed from the rows **this run** wrote via `ListPipelineStages`, not from `pipeline.Flow`, and it excludes its own stage since it cannot know its own outcome yet. An empty or unreadable ledger returns an error that becomes `skipped`, never a zero-valued pass — the vacuity guard `PIPE-010` and `PIPE-011` established.
  - Three discrimination proofs, each by breaking the guarded code and watching the right test fail: the `Section33Resolved` lookup, the empty-ledger guard, and the nil-store branch.
  - **Outstanding:** one `ledger.decide(ctx, pipeline.StageSkipAudit, checkSkipAudit(...))` call in `agent_execution.go`, before `ledger.close(ctx)` so `close` does not sweep stage 39 as not-implemented first. Held back because another lane is actively editing that file. Landing it will also change `engine_pipeline_ledger_test.go`'s integration assertion of zero not-implemented rows.
...
- [ ] `PIPE-045 DATA` Renumber the flow to sixty-six stages and prove that recorded rows from the previous numbering still read correctly by name, including a migration test over rows written under the old numbers.
  - **Declined, with the hazard traced against live code rather than assumed.** The blocker is not the numbering itself, it is that a stage's *classification* is re-derived at read time from the raw persisted number: `pipeline.ClassifyDecline(stage Number, state State)` looks the number up against whatever `Checks`/`Section33Resolved` the **currently running binary** declares, not against anything stored on the row.
  - Two production callers read historical rows long after they were written: `internal/coordinator/skip_audit.go:183` and — decisively — `web/frontend/pipelineledger/model.go:224`, which recomputes classification for **every row of every historical ledger a person opens**, from wire-carried `(Number, State)` alone. Both confirmed by reading.
  - So after a renumber, a row recorded as stage 38 keeps its correct `stage_name` (baked in at write time by `RecordPipelineStageResult`) while its classification silently describes whatever stage 38 means in the *new* table. **Name and reason would disagree on the same row and nothing would notice.**
  - Two further obstacles: migration `000031` bounds `stage_number` with `CHECK (BETWEEN 1 AND 64)`, insufficient for 66 regardless; and `pipeline_stage_records_immutable_update` raises `ABORT` on any `UPDATE`, so a data migration rewriting old rows cannot be a simple `UPDATE` without defeating an immutability guard `AGENTS.md` requires preserving.
  - The real fix is to rekey `ClassifyDecline` and the wire by stage **name** rather than number — a design change in `skip_audit.go` and `model.go` — or to write a rows-copy migration that respects immutability. Neither was done. **No renumbering was applied.** Follow-up: `PIPE-045a`.
- [ ] `PIPE-046` Add declared run profiles marking stages inapplicable before the run starts, so a library run declines crash-recovery in advance rather than skipping it at runtime, and make `PIPE-042` audit which of the two happened.
  - **Built in `internal/pipeline/profiles.go`, and not yet consulted by any run.** `RunProfile`/`ProfileDecline`, `ProfileDefault` (declines nothing), `ProfileLibrary` (declines `StageAdversarial` and `StagePlatformMatrix` — the two stages the plan's own "pure library" example actually reaches in the current 41-stage flow; crash-recovery and compatibility do not exist as stages yet because `PIPE-045` is blocked), plus `ApplicableStages`, `DeclinedReason`, `ProfileFor` and a `ValidateProfiles` following the `ValidateChecks`/`ValidateRequirements`/`ValidateResourceClasses` pattern.
  - `decline.go` is extended additively with `DeclineByProfile` and `ClassifyDeclineForProfile`, which returns the new class when the profile declines the stage and otherwise falls through to the unchanged `ClassifyDecline`. `ProfileDefault` is provably inert: `TestPIPE046_DefaultProfileNeverOverridesAnything` sweeps every stage against every state.
  - **The discriminating test drives the real scheduler**, not a model of it: `TestPIPE046_AProfileSelectsADifferentStageSetFromTheSchedulerItself` runs `RestrictToStages` + `RunConcurrently` once per profile and proves the Runner is **never invoked at all** for the two declined stages under `ProfileLibrary`, while all 41 are invoked under `ProfileDefault`. Verified independently: six profile tests pass.
  - **Zero production call sites, so the ticket stays open.** Every place stage selection or classification actually happens sits in files the lane did not own: `agent_stage_runner.go`, `skip_audit.go`, and `web/frontend/pipelineledger/model.go`. This is the fifth subsystem this session that is complete and uncalled; it is flagged rather than claimed live. Follow-up: `PIPE-046a`.
- [x] `PIPE-047 DOC` Record in `docs/plan.md` §33 which stages each shipped profile declines and why, so a profile cannot quietly become the way a stage stops being performed.
  - **Closed.** A new "Declared Run Profiles, As Shipped (PIPE-046/PIPE-047)" subsection in `docs/plan.md` §33 records which flow it applies to (the current 41-stage one, since `PIPE-045` is blocked), what each shipped profile declines and why, and — directly answering this ticket's own worry that "a profile cannot quietly become the way a stage stops being performed" — that its integration status is *declared, not yet consulted*, naming the exact three call sites that would make it live.
...
- [ ] `PIPE-045a DATA BLOCKER` Rekey decline classification by stage name rather than by number, so the flow can be renumbered at all. `pipeline.ClassifyDecline` re-derives a historical row's classification at read time from its persisted `Number` against the running binary's tables, and `web/frontend/pipelineledger/model.go` does this for every row of every ledger a person opens. Until the key is the name — which `RecordPipelineStageResult` already bakes into each row — any renumber makes old rows' names and reasons disagree silently. Also needs migration `000031`'s `stage_number BETWEEN 1 AND 64` widened, and a rows-copy migration that respects the `pipeline_stage_records_immutable_update` guard rather than defeating it.
- [ ] `PIPE-046a` Consult the declared run profile from the three places stage selection and classification actually happen: `agent_stage_runner.go`'s scheduler call, `skip_audit.go`'s `classifySkipAuditEntry`, and `web/frontend/pipelineledger/model.go`'s `classifyDecline`. `PIPE-046` ships the table, the validator and `ClassifyDeclineForProfile` with a test that drives the real scheduler, but nothing reads any of it, so no run declines anything in advance yet.
- [ ] `REPO-041a TEST` Point the four graph-canvas checks at `/graphs` instead of `/tasks`. The graph canvas moved to a dedicated route and, at wide viewport, to the observation rail; `wide-graph-visible`, `graph-canvas-mounted-high-dpi-accessible-overlay`, `graph-canvas-viewport-selection-and-deliberate-inspection` and `graph-canvas-responsive-visible-bounds` assert it exists inline on `/tasks` and are unsatisfiable there by design.
- [ ] `REPO-041b BUG` Task selection is lost on viewport resize. After cycling compact→standard→wide→minimum, `/tasks` reverts to "No task yet" while the transcript stays populated. Surfaced by `graph-selection-survives-narrow-tabs-and-viewport-churn` once a real thread existed to select; screenshot captured under the REPO-041 run artifacts.
- [ ] `REPO-041c` Make the seed server's cleanup orphan-safe. `StartMountedThread`'s `defer Shutdown` only runs on a clean exit, so a hard kill leaves the coordinator and its worker holding `.artifacts/seed/mounted/coordinator.sqlite3` and `codeflux-worker.exe`, and the next `seed` fails its `os.RemoveAll` with `unlinkat ... Access is denied`. Reproduced three times on 2026-08-02. A stale-lock sweep at startup, or a pidfile the next run can reclaim, rather than relying on the defer.
- [x] `PIPE-048 BLOCKER DATA` Add the atom-registration stage and write the registry row: documentation, contract hash, signature shape, evidence links, and the exact revision the atom was verified at. No run writes the registry today, so recall has nothing to search.
  - **Closed.** A new `StageAtomRegistration` writes real `atom_documentation_revisions` rows during a run: `TestPIPE048_RegisterVerifiedAtomsWritesARealRegistryRow` reads the row back and checks `ContractHash == recallkey.ComputeContractHash(...)` and `SourceRepositoryRevision == scope.revision`. An atom carrying only an ordinary comment registers nothing (`TestPIPE048_UndocumentedProducedAtomIsNotRegistered`).
  - **Identity is proven in both directions, which is what makes the reuse claim mean anything.** Same contract registered twice yields the same `AtomID`; a genuinely different contract in the same project yields a **different** one, with no contamination either way. Stated discrimination: a `resolveRegistrationIdentity` that ignored the contract-hash key and simply returned the most recent atom would pass the same-contract test — only one prior revision exists there — and fail the changed-contract one.
...
- [x] `PIPE-049 DATA` Add molecule-documentation and molecule-registration on the same terms, recording what the composition guarantees and the atoms it composes as its parts, so a later run can reuse joins and not only leaves.
  - **Closed.** `StageMoleculeRegistration` registers a molecule on the same terms as an atom and names its parts: `TestPIPE049_RegisterVerifiedMoleculesNamesItsComposedAtoms` shows an uncommented `addOne` registering nothing while the molecule calling it registers with `DependencyBindings == [{Name: "addOne", Version: scope.revision}]`.
- [ ] `PIPE-050 BLOCKER` Move recall ahead of the atoms stage and make it binding: every contract carries a `reuse` or `write` decision before anything is built, and a `reuse` decision names the registered atom, its evidence, and its verified revision.
  - **Binding is implemented; the execution-order half is not, and is blocked on a held file.** Every contract now leaves `recallKnownAtoms` with a `recallDecision`: either `reuse`, naming the matching project function, its artifact ID and the contract hash that matched, or `write`, carrying a justification — "no project function shares this contract", or a specific statement of how many shape-only candidates were rejected and why. `bindRecallDecisions` **fails the stage** naming any contract left without a decision, which is what makes it binding rather than a list nobody must answer to.
  - **Still not moved ahead of the build.** `recallKnownAtoms` runs after assembly, integration tests, the adversarial probe and end-to-end tests — so it still reports on code already written. Moving it means deriving contracts from the plan before the attempt loop starts, which restructures `Run` in `agent_execution.go` and `examineStructure` in `agent_stage_runner.go`; both were held by an active lane. `pipeline.Checks` keeps its `Unestablished: "PIPE-050"` marker for `StageRecall`, honestly, because the gate is not yet established.
- [x] `PIPE-051 BLOCKER` Implement the contract-hash key: an exact match on the normalised signature with its preconditions, postconditions, and effects, answered from the index with no model request, replacing the `strings.Contains(content, "func "+name+"(")` match that finds a renamed atom never and a same-named different atom always.
  - **Done.** `ComputeContractHash` in the new `internal/retrieval/recallkey`: SHA-256 over the name-erased parameter and result type vector, `ReturnsError`, and sorted deduplicated effects. An index lookup, no model request. It replaces a `strings.Contains(content, "func "+name+"(")` match, and the two new tests state the defect exactly — a rename was found **never**, a same-named different contract **always**.
  - **Proven to discriminate** by temporarily replacing the contract-hash lookup with a name-only key, reproducing the old defect: both tests failed with those exact symptoms, then passed again on revert.
  - **A package boundary worth recording.** `internal/retrieval`'s own `gate_evidence_g07_test.go` fails the build if any production file there imports `internal/atomdoc`, keeping the M21 discovery gate from reading documentation semantics. The contract hash legitimately needs `atomdoc.ContractHash`'s validated type but belongs to the pipeline's recall stage, not M21 discovery — so it went in a sibling package rather than tripping a guard whose purpose it does not share. The guard was run before and after the move to confirm that reading.
- [x] `PIPE-051a` Implement the signature-shape key: the parameter and result type vector with names erased, indexed, returning a bounded candidate set rather than a verdict.
  - **Done, and deliberately incapable of producing a verdict.** `SignatureShape` erases names from the type vector and is used in exactly one place: to produce a candidate **count** for a write decision's justification prose. `bindRecallDecisions` reads the shape index once and can only turn it into text, never into a `reuse` verdict — so `AGENTS.md`'s rule that similarity never establishes permission is enforced by structure rather than by convention.
- [ ] `PIPE-051b` Implement the documentation-embedding key over the shape candidates only and never over the whole registry, so similarity proposes and the contract hash disposes, as §7 requires.
  - **Correctly still blocked, and untouched.** `cmd/codeflux-dev/vector_branch_test.go` still fails if any production file calls a vector writer, and `AUDIT-026`'s precondition — the deterministic-retrieval miss measurement over reviewed real-run fallbacks — still cannot run, because no reviewed fallback exists. Nothing embedding-related was implemented and `TestAUDIT026` passes unmodified.
  - **No end-to-end reuse is demonstrated, and the report says so rather than implying otherwise.** The registry is empty until `PIPE-048` lands, so recall's only search surface is the project's own prior stored source, which carries no evidence link and no verified revision. A `reuse` decision states that limitation instead of inventing either.
- [x] `PIPE-052` Admit a recalled atom only once it passes this run's synthesised cases from the case-synthesis stage, and record that re-verification as the atom's evidence in this run, so reuse cannot import the earlier run's blind spots.
  - **Closed, with the affirmative case as well as the refusal.** A seeded `StageAtomVerification=Failed` row for this attempt downgrades a would-be exact contract-hash match to `write`, and the justification names the ticket.
  - The reason there are three tests and not one: a gate tested only by its refusal is indistinguishable from a gate that refuses everything. `TestPIPE052_ReuseIsAdmittedWhenThisRunsVerificationExplicitlyHeld` supplies the other direction — without it, a defective gate that downgraded on *any* checked evidence rather than only on failure would pass every other test in the file. `TestPIPE052_ReuseStillAdmittedWhenNoVerificationEvidenceExists` pins the fail-open default so PIPE-050/051's binding is untouched.
...
- [x] `PIPE-054` Keep `recallKnownAtoms` as the reuse-regret stage at the end of the flow, never blocking, comparing every function the run wrote against the registry as it now stands and recording which of the three keys would have found the match.
  - **Closed together with `PIPE-055`.** `TestPIPE054_055_ClassifiesRegistryGapVersusRecallMiss`: a function sharing nothing with any prior work classifies as `registry-gap`; a function whose contract was registered but not stored as a plain artifact classifies as `recall-miss`. Both appear, neither collapses into the other, and both index by the right function name. "Nothing was reused" stops being one undifferentiated outcome.
...
- **Registry population landed but the reuse decision does not read it — `PIPE-G08` is NOT demonstrated.** `bindRecallDecisions`, stage 6's binding matcher, still searches only `ListProjectSourceArtifactsExcludingTask` and does not consult the new `atom_documentation_revisions` registry. So the registry is now written on every run and correctly keyed, but nothing reuses out of it yet. Widening that search is the concrete next step, and it is exactly what the `recall-miss` classification above is pointing at: "the registry has what recall's own search didn't find."
- **`Run()` reaches both performers, but not through the ledger.** `recallKnownAtoms` — already called from `Run()` whenever the produced code compiles — now calls `registerAndRecordProducedDeclarations`, so the performers genuinely execute and genuinely write rows. What does not happen is `ledger.decide(StageAtomRegistration, …)` through the `pipelineLedger` object; that call site lives in `agent_execution.go`, which the lane did not own. `recordRegistrationStageResult` writes the stage's own `pipeline_stage_records` row directly and idempotently instead, documented as a narrow exception to this package's ledger-is-the-only-writer shape. The `PIPE-048`/`PIPE-049` unestablished markers in `checks.go` stay up and are the accurate signal of exactly that gap.
- [ ] `PIPE-056 DATA` Record influence through the existing `retrieval.RecordInfluence` path when a recalled atom is accepted or rejected, so reuse decisions are auditable and an invalidated atom can be re-derived transitively as §31 requires.
  - **Blocked, and deliberately not stubbed.** `retrieval.RecordInfluence` accepts only an `InfluentialMemoryItem` whose `decision` field is unexported and populated solely by `retrieval.Service.RunPreWorkGate`. `AgentExecution` holds no `*retrieval.Service`; the only construction site is `application.go` and it hands the service to `taskPreflightService` instead. Wiring it means editing `AgentExecution`'s struct and constructor in `agent_execution.go`, which the lane did not own. Speculative uncalled plumbing in `internal/retrieval` was rejected as worse than an honest gap.
...
- [x] `PIPE-057` Parse the produced source once per run and share the tree: `readProducedFunctions` is called twenty times and `producedGoFiles` thirteen more, each re-shelling to `git status` and re-parsing identical content.
  - **The ticket's premise does not hold, and the measurement says so.** Instrumented against the engine fixture: `parseProducedFunctions` costs **0–506µs** per call — AST-parsing one small file is essentially free. The real per-call cost is `producedGoFiles`, a `git status` **process spawn**, at ~18–22ms. Six calls of each is about 120ms of a 3.8s run, roughly 3%. So the thing worth sharing is the shell-out, not the tree.
  - **A wasted parse found and removed:** `resolveAttribution` built an `attributedScope` that **both** of its callers discarded (`attribution, _ := …`). Once per run and once per attempt, entirely thrown away.
  - `producedFunctionCache` is scoped to a **single call** to `examineStructure` and never persists past it — not across an attempt, not into `outstandingWork`. Nothing between the four cached checks writes to the worktree, so the cache is provably valid for exactly that window and is gone before mutation testing, which does write to disk.
  - **Keyed on the exact sorted file list, never on the worktree**, which is what stops a `producedGoFiles`-derived list and an attribution-derived list from silently sharing an entry. That conflation would have been a correctness bug rather than a performance one, and there is a test for it.
  - Deliberately not cached: `checkSimplification` runs *after* mutation testing writes and restores files in place, so spanning that boundary would be unsafe if a restore ever failed part-way. The rest have callers in files other lanes held.
  - **Wall clock is reported as noise-dominated rather than dressed up.** Five lanes were editing the shared checkout throughout; a controlled interleaved A/B drifted together as load rose, and the provable ~60–140ms saving is below this machine's current noise floor on a multi-second test. The structural claim — four shell-outs collapse to one — is guaranteed by the code, not sampled.
- [x] `PIPE-058 BLOCKER` Declare `Requires` edges on every stage as data in `internal/pipeline`, with a test that the graph is acyclic, that every stage is reachable, and that no edge points forward in the flow.
  - `internal/pipeline/requirements.go`: a `[]Requirement` slice beside `Checks`, not a `Stage.Requires` field — `Flow`'s entries are positional composite literals, so a new field means rewriting all 38. A slice rather than a map because coverage is validated against `Flow`, so a stage added by another lane **fails loudly instead of silently defaulting to no requirements**. That happened during the session: `PIPE-020` added stage 38 and the table refused it until an entry was written.
  - **The widest frontier the graph permits is 5** — `integration-tests`, `end-to-end-tests`, `global-invariants`, `adversarial`, and `platform-matrix` all unblock the instant `program` is satisfied. That number is the point of the ticket, and a test asserts the exact set by name and fails if the table degenerates to a chain.
  - Non-obvious edges, each carrying its reason in the source: `recall`, `atom-case-synthesis` and `atom-property-tests` are siblings under `contracts`, none needing the others; `atom-fuzz` requires only `atoms`, since fuzzing exercises parsing boundaries rather than the suite; `path-coverage` and `assembly` are siblings, coverage being a measurement of the finished state rather than a repair; `end-to-end-tests` requires `program` **and** `instructions` directly, a genuine long-range edge skipping 26 stages because it reproduces the example recorded at intake; `evidence-bundle` requires all five Phase F stages rather than `program`, matching "reads the ledger, not the worktree".
  - **Assumption flagged for review by its author rather than buried:** "the suite" in the `repetition` and `non-functional` gates was read as the executable-level suite (`integration-tests` + `end-to-end-tests`) rather than literally every test stage. Defensible, not the only reading.
  - All five validations proven to discriminate against hand-broken tables, including a real injected 3-stage cycle whose report names all three stages rather than saying only that a cycle exists, and two distinct unreachability scenarios. The validators are parameterised so tests build broken tables without touching package state.
- [x] `PIPE-058a BLOCKER` Execute the declared frontier concurrently while the ledger continues to record by stage number, so execution order and reporting order stop being the same thing.
  - **Closed. `examineStructure` now schedules through `pipeline.RunConcurrently`**, restricted to its own stage list by a new `pipeline.RestrictToStages`: 19 unconditional stages and 5 suite-verified ones.
  - **Measured frontier is 4-wide** for the unconditional set across 14 waves (`TestPIPE058a_MeasuredFrontierWidthAgainstTheRealTables`), and mutation-alone-then-4-wide for the verified set. That is against a theoretical whole-graph width of 5; the three stages accounting for the difference (integration-tests, end-to-end-tests, adversarial) are decided in `agent_execution.go`, outside this lane's ownership. The lane flagged the boundary rather than crossing it.
  - **PIPE-006's documented limitation is now repaired, which this ticket had to do:** `agent_pipeline_ledger.go` gained `recordMeasured`/`recordDecision`, stamping each stage's start from the scheduler's own per-goroutine bracket around that stage's `Runner` call, instead of inferring it from when the previous stage was recorded — which stops meaning anything the moment stages overlap. A blocked stage reports `Measured=false` honestly rather than a fabricated near-zero span. `State`, `Detail` and `Evidence` are byte-identical to before; the sequential `record` path is untouched for `agent_execution.go`'s callers.
  - **Casualty found and fixed at the right level.** The refactor broke `TestPIPE017_OptimizationIsDecidedAfterMutation`, which asserted ordering by grepping `agent_stage_runner.go` for two literal call-site substrings that the rewrite deleted. It was replaced rather than re-pointed at new literals: a static check against the real `PlanWaves` output over the real `Requirements` graph, plus `TestPIPE017_OptimizationIsObservedAfterMutationAtRuntime`, which calls `RunConcurrently` and asserts on measured `Decision.StartedAt`/`FinishedAt` — optimization never starts before mutation finishes. Both pass at `-count=10` with no flakes.
  - **The ordering guarantee was traced, not assumed.** It comes from `requirements.go`'s pre-existing `AtomOptimization → AtomMutation` edge, which survives `RestrictToStages`, **not** from mutation's exclusivity. Exclusivity keeps mutation away from atom-fuzz, repetition and non-functional — which read the worktree via `go test` while mutation is rewriting it — and would not by itself order optimization after it. Concurrency dropped no ordering the flow relies on.
  - Verified independently: build, `gofmt`, `vet`, full `internal/pipeline` suite, and the PIPE-017/057 tests all clean. `-race` is unavailable on this windows/arm64 host per `AGENTS.md`, so the mutex is not race-detector-verified — a real gap in the evidence, not a formality.
  - **Left undone deliberately:** `PIPE-060`, `PIPE-061`, `PIPE-062` and `PIPE-065`, each needing files another live lane owned (`agent_stage_mutation.go`, `internal/storage/**`) or restructuring with real regression risk against an actively-changing tree.
  - Engine canary at 5.2–5.4s against a 3.8s baseline. The lane could not separate scheduling overhead from contention with five other agents on this machine and said so rather than picking the flattering reading. **Worth re-measuring on a quiet machine** before treating concurrency as a speed win.
- [x] `PIPE-059` Declare a resource class per check — pure AST, build-only, suite-read, exclusive-mutating — and schedule against it, since the worktree rather than the logic is what forces serialisation.
  - **Closed.** `internal/pipeline/resource_classes.go` declares four classes — pure-ast, build-only, suite-read, exclusive-mutating — assigned per stage by **reading each check's actual `exec.Command`/`os.WriteFile` calls** rather than inferred from stage names. Only `checkMutations` is exclusive-mutating: it is the one checker that writes a produced source file in place and restores it. `TestPIPE059_ExclusiveMutatingStageNeverSharesAWaveWithAnything`, repeated 25 times, holds the scheduler to it.
  - Consequence handled rather than discovered later: the four pure-AST checks sharing `producedFunctionCache` (`checkAtoms`, `checkAtomTests`, `checkMolecules`, `checkAtomDocumentation`) now run in the same wave, so the cache's four memoised fields are guarded by a `sync.Mutex`. PIPE-057's existing cache tests pass unmodified.
...
- [x] `PIPE-064 TEST` Prove a stage's verdict does not depend on how many stages were in flight: run the same worktree serially and in parallel and require identical ledgers apart from timing.
  - **Landed first, before any stage ran in parallel, which is the order the section demands.** `internal/pipeline/scheduler.go`. `TestPIPE064_AStageVerdictDoesNotDependOnConcurrency` proves `RunConcurrently` gives identical verdicts at `MaxWorkers:1` and `MaxWorkers:0`. The companion `TestPIPE064_ConcurrencyActuallyHappens` **deadlocks if the scheduler secretly serialises a wave**, so "identical either way" is a claim against genuine concurrency rather than against accidental seriality — without it the property would be trivially true and worthless.
...
- [ ] `PIPE-066 BLOCKER` Route every model request through one admission point with a configured maximum concurrency, so fan-out cannot exceed what the provider tolerates and the ceiling is a setting rather than an accident of the graph shape.
  - **Read this before any ticket below: the production model path does not go through `internal/providers` at all.** `internal/openaimodel.Model.ObserveThink` — the only implementation of `agent.FixedModel`, wired in by `newDefaultAgentModel` — makes a bare `model.options.Client.Do(request)` against the Responses API at `model.go:141`. No retry, no backoff, no `Retry-After`, no admission control, no budget check. Verified independently by symbol search: the **only** caller of `NewRetryExecutor`/`BudgetedProviderExecutionService` anywhere in production is `cmd/codeflux-dev/live.go`, the `run-live` smoke gate, which issues one serial request.
  - So everything below is real, tested, load-bearing infrastructure on the port that exists — and **none of it is in the path a real run spends money through.** That is a materially different fact from "the admission point is built," and it is the reason `PIPE-066`, `PIPE-069` and `PIPE-070` stay open. Filed as `PIPE-066a`.
  - The grant named `internal/agentloop/**`, which does not exist — the real loop is `internal/agent`, imported under that alias in `agent_execution.go`. No empty package was created to match the name.

  - `internal/providers/admission.go`: `AdmissionController`, a resizable semaphore (token channel plus drain-debt for narrowing), one per `RetryExecutor`, with `Acquire`/`release` wrapping every physical attempt in `Execute`. `RetryPolicy.MaximumConcurrency` defaults to 4 and validates 1–64.
  - Discriminating: `TestAdmissionControllerBoundsConcurrentAcquisitions` and `TestRetryExecutorAdmitsEveryPhysicalAttemptThroughOneCeiling` launch more goroutines than the ceiling and assert observed peak concurrency equals the ceiling exactly — removing the `Acquire` call takes them from 2 to 6.
  - **Stays open:** the engine loop does not call this port. See the note above.
- [x] `PIPE-067 BLOCKER` Treat `429` and provider overload responses as backpressure: exponential backoff with jitter, honouring `Retry-After` when the provider sends one, recorded as a wait rather than a stage failure.
  - **Closeable, and most of it predated the ticket.** `retry.go` already honoured `Retry-After` and backed off exponentially. What it did not have — and what the ticket and §"Rate-limit responses are backpressure" explicitly require — is **jitter**. Added `RetryPolicy.Random` and equal jitter over `[base/2, base]`.
  - Discriminating: `TestRetryExecutorAppliesEqualJitterWithinBounds` asserts backoff varies with the injected random source, which fails against the old fixed schedule because it ignored `Random` entirely. The one pre-existing test asserting an exact sleep schedule was **pinned with `Random: 1` and a comment rather than deleted.**
- [x] `PIPE-068` Bound retries by attempt count and by total wall clock, then record the stage blocked with the provider's own reason, because "we could not ask" and "we asked and the answer was no" are different facts in an immutable ledger.
  - **Closeable on pre-existing evidence, and no behavioural change was made.** `MaximumAttempts`, `MaximumCumulativeWait`, an `AttemptRetryExhausted` disposition distinct from `AttemptFailed`, and `ErrRetryBudgetExhausted` distinct from a normal failure were all already implemented and correct — "we could not ask" and "we asked and the answer was no" are already different facts here. Verified, not rebuilt.
- [ ] `PIPE-069` Narrow the concurrency ceiling for the remainder of the run under sustained rate limiting instead of retrying at full width into a limit.
  - `isProviderBackpressureFailure` (rate-limited or overload) triggers `AdmissionController.Narrow(ceiling-1)` on every such `AttemptRetryScheduled`. `Narrow` is monotonic — it never widens — and floors at 1.
  - Tested directly (`TestAdmissionControllerNarrowRetiresSlotsRatherThanWidening`) and end to end against a scripted 429 server, both subtests asserting the ceiling drops by exactly one per rate-limited retry.
  - **Stays open** for the same caller gap as `PIPE-066`.
- [ ] `PIPE-070` Enforce budget and forecast at the admission point so concurrent stages cannot each believe they are inside the cap while collectively exceeding it.
  - **Left open, and the honest reason is a layering one.** Real budget enforcement already exists — `provider_budget_execution.go`'s `ReserveProviderBudget` takes an optimistic-concurrency revision-CAS reservation *before* `executor.execute()`, so two concurrent stages cannot both believe they are under the cap: one wins the CAS and the other gets `ErrStaleRevision`. That is exactly the property this ticket asks for.
  - But it happens at the ledger level in `internal/coordinator`, not *at* the admission point, and pricing and storage types do not belong in `internal/providers`, which is deliberately free of both. No new budget logic was invented to manufacture a closure; moving the check to the admission point needs coordinator changes outside the lane's grant.
- [x] `PIPE-071 TEST` Add a provider fake returning `429` with and without `Retry-After` and assert the backoff schedule, the ceiling reduction, the ledger wording, and that no stage records failed because of a rate limit.
  - **Closed.** `TestRetryExecutorRateLimitBackpressure` drives a scripted 429-returning `httptest.Server` with and without `Retry-After`, and asserts all four things the ticket names: the backoff schedule (`Retry-After` honoured exactly and unjittered; the absent-header path jittered with an injected source), the ceiling reduction of exactly one per rate-limited attempt, the ledger wording, and — checked explicitly — that **no `AttemptRecord` ever carries `AttemptFailed` for a rate-limited outcome.**
- [ ] `PIPE-072 UX` Show waiting-on-provider as its own state in the run timeline, distinct from working and from blocked, so a throttled run does not read as a stalled one.
  - **Not done, and the data for it already exists.** `AttemptRetryScheduled` is already distinct from `AttemptFailed`, `AttemptRetryExhausted` and `AttemptSucceeded`. Surfacing it as its own run-timeline state is a `web/frontend` and pipeline-ledger change, neither of which this lane owned.
- [ ] `PIPE-066a BLOCKER` Route the real model path through the admission point. `internal/openaimodel.Model.ObserveThink` — the only `agent.FixedModel` implementation, and the one a real run uses — issues a bare `http.Client.Do` at `model.go:141` with no retry, no backoff, no `Retry-After` handling, no concurrency ceiling and no budget reservation. `internal/providers.RetryExecutor` and `coordinator.BudgetedProviderExecutionService` implement all of it and their only production caller is `cmd/codeflux-dev/live.go`'s one-shot smoke gate. Until this lands, a real code-generation run has **no rate-limit handling of any kind**, and `PIPE-066`, `PIPE-069` and `PIPE-070` are unenforceable however well they are implemented. This is the single largest gap between what the provider section claims and what a run does.
...
- [x] `PIPE-111 BLOCKER` Derive the changed line ranges from the base revision the worktree was created at, replacing the changed-file list as the run's unit of attribution.
- [x] `PIPE-111a BLOCKER` Map those ranges to their enclosing declarations and expose the attributed set to every check, so a one-line fix inside a thirty-function file does not make the run answerable for the other twenty-nine.
  - **Done.** `internal/coordinator/agent_stage_attribution.go` derives `changeAttribution` from the base revision in the durable worktree binding -- never from `git status` or `HEAD` -- via a single `git diff --unified=0 <base>`. One revision, deliberately: comparing the base against the working tree folds together what the run has already committed to its own worktree and what is still uncommitted, which is exactly the case a two-revision diff would miss. Untracked new files are attributed in full.
  - **Zero attribution and failed attribution are separate booleans, not an empty map.** `Contains()` and `Touches()` fail toward **inclusion** when `Established` is false -- an unresolvable base, a git error -- while a genuinely empty *established* attribution excludes everything, which is `PIPE-124`'s refactor case. Pinned directly by `TestPIPE111a_ZeroAttributionIsDistinguishedFromFailedAttribution` and by a test that drives a real unreadable base revision.
  - **A real defect found on the way, and it is worse than the one this ticket describes.** Every check enumerated produced files through `producedGoFiles`, which reads `git status --porcelain` and therefore sees only **uncommitted** edits. The moment a run commits to its own worktree -- the documented operating mode -- that mechanism goes completely blind rather than merely over-broad. File enumeration is now explicit everywhere the rescoped checks need it, with a separate whole-worktree walk for *test evidence* lookups, since a test proving a declaration works may legitimately live in a file the run never touched.
  - `PIPE-115`'s unestablished marker on `StagePathCoverage` is cleared, because the scoping defect it named is fixed: coverage is now measured over changed lines by parsing the raw profile. A comment records the remaining wording gap -- the gate says "branch coverage" and the check measures statement coverage -- as pre-existing and out of this ticket's scope rather than silently conflating the two.
  - **`PIPE-116` is what makes the other six believable.** A twelve-function, two-file fixture repository with real tests, a genuine anti-pattern (a discarded error) and a genuine completeness gap, both in the same file a later one-line edit touches. The test commits it as base, applies one semantically-neutral edit, and asserts exactly one of twelve declarations is attributed -- then proves the pre-existing finding and gap are recorded as context or skipped rather than gating. A clean-slate fixture cannot demonstrate that, because it has nothing pre-existing to disagree about.
  - Every test carries an explicit contrasting run -- unestablished attribution, or a constructed wide-open attribution simulating the old file-level scoping -- and asserts the defect reproduces on the same fixture before asserting the fix. Two real bugs were caught that way, including the committed-worktree blindness above.
- [x] `PIPE-112` Re-scope the completeness loop to attributed declarations only, so the refinement loop stops sending a run back to write tests and doc comments for functions it never touched.
- [x] `PIPE-113` Re-scope anti-patterns, complexity, and simplification to attributed declarations, and report a pre-existing finding in a touched file as context rather than as a gate failure.
- [x] `PIPE-114` Re-scope mutation to attributed lines, so the score measures whether this change's tests detect this change's defects.
- [x] `PIPE-115` Report coverage for the changed lines rather than the least-covered package in the module, which on any real repository describes code the run never touched.
- [x] `PIPE-116 TEST` Add a fixture repository with substantial pre-existing code and prove that a one-line change is judged on one declaration, since every gate in phases B through D inherits this scoping.
...
- [x] `PIPE-117 BLOCKER SPIKE` Research how each of the six declared task classes states an executable acceptance example. The current form is arguments, stdin, and expected output, which only a command-line filter can satisfy; a refactor, a dependency change, a library feature, and behaviour-linked documentation can express none of them, and `PIPE-019` makes an example mandatory.
  ...
  - Verified: `go test ./internal/pipeline/ ./internal/coordinator/ -run 'TestPIPE|TestAUDIT|TestEveryStage'` green.
- [x] `PIPE-118` Add a second example form for work with no command-line surface: a named test that must pass, identified by package and test name, so a library change can be judged against something the requester wrote.
  - `internal/coordinator/agent_acceptance_named_test_form.go` adds the form. It shares the `<<<ACCEPTANCE` markers with the command form and is told apart by a `test:` line, so a requirement states one vocabulary rather than two. `package:` is optional and defaults to the whole module, which is the honest default for a requester who knows the test name but not where it will live.
  - Wired into `checkAcceptance` **before** the command path and independently of it: named tests are the form work with no command-line surface can state, so requiring a runnable command first would put the general form behind the specific one.
  - Two mistakes this form invites, both closed:
    - The `-run` pattern is anchored on both sides. Unanchored, `TestParse` would be satisfied by `TestParseRejects` passing while `TestParse` itself failed.
    - **A pattern matching nothing exits zero.** `go test -run TestMisspelled` prints `ok … [no tests to run]`, so a typo would satisfy the example while checking nothing. Detected explicitly and reported as a failure.
  - Six tests: parsing with and without a package, three non-test names refused and the three Go test prefixes accepted, a passing test reported passed, a failing one reported failed, a name matching nothing reported as a failure rather than a pass, and the anchoring proved against a prefix sibling.
...
- [x] `PIPE-127 BLOCKER` Build every mutant before running the suite and discard the ones that do not compile: `" + "` becomes `" - "` inside string concatenation, the build fails, the suite fails, and the score counts it as caught, so the current score is inflated by mutants that never ran.
  - **Done, and the old implementation was worse than the ticket says.** Each candidate is now written, built with `go build ./...` **before** the suite ever runs, and discarded — file restored, `mutations_discarded_uncompilable` incremented, `applied` and `caught` untouched — when it does not compile. On a real fixture comparison, one of the old implementation's five "caught" mutations **was a build failure it credited as caught**. That is the free score this ticket exists to remove, observed rather than theorised.
  - Discarding is also **~6x cheaper** than scoring: ~0.11s for a failing build against ~0.73s to run one through the suite, and that ratio widens on a slower suite.
- [x] `PIPE-128 BLOCKER` Mutate the syntax tree rather than the file bytes, so a substitution cannot land inside a string literal or a comment and produce a survivor that is not a blind spot.
  - **Done, and it is structurally impossible rather than merely avoided.** `discoverASTMutationCandidates` parses with `go/parser`, walks with `ast.Inspect`, and matches only `*ast.BinaryExpr` nodes whose operator is one of ten mutable tokens, splicing at `binary.OpPos`. A string literal is one opaque `*ast.BasicLit` and a comment is wired into no expression, so neither can ever produce a `BinaryExpr`.
  - **Proven by contrast rather than by assertion:** the test places the same `" == "` byte pattern in a comment, a string literal and real code, then runs both discoveries side by side. The AST finds only the real code; the retained byte scanner lands on the comment — reproducing the exact defect in the same test that shows it fixed.
- [x] `PIPE-129` Sample mutants across the attributed lines rather than taking the first occurrence of each pattern in each file, so the score is not an artifact of file ordering.
  - **Done.** An evenly spaced stride across the sorted candidate list rather than the first `budget` entries, so with more candidates than the twelve-mutation budget the sample reaches the back of the attributed range instead of clustering at its head. Tested with 40 candidates against a budget of 12, asserting the sample differs from the first twelve and spans both halves; a second test guards the within-budget no-op.
- [x] `PIPE-130 TEST` Prove the score's meaning with a fixture whose tests are known to be blind and one whose tests are known to be thorough, and record the false-positive and false-negative rates as §31 requires of a mechanical rule.
  - **Done, and this is what makes the other three believable.** A fixture whose single test calls `WithinLimit(3, 5)` and **discards the result** — genuinely blind to the boundary — scores **0%** and records `broke`: a high score is unachievable there by construction. The contrast is identical source with a test that checks both edges, scoring **100%** and holding. Without that pairing, a low score would be indistinguishable from a broken check.
  - **Not done, and flagged rather than glossed:** this ticket's full text also asks for false-positive and false-negative *rates*, which need repeated trials across many fixtures. Only the blind-versus-thorough proof was delivered. Filed as `PIPE-130a`.
  - On that basis `StageAtomMutation`'s `Unestablished: "PIPE-127"` marker is cleared: the gate claims the tests are known to detect defects, and the free-score path, the unkillable-comment path and the ordering artifact are all gone while the score is shown to discriminate blind from thorough tests.
...
- [x] `PIPE-130a TEST` Measure the mutation score's false-positive and false-negative rates across many fixtures. `PIPE-130` proved the score **discriminates** — a blind-test fixture scores 0% and a thorough one 100% on identical source — which establishes the mechanism works, but not how often it is wrong. Rates need repeated trials over a fixture population, and until they exist the threshold is a number nobody has calibrated. Related to `PIPE-105`, which asks the same question of the critic's finders.
  - **Closed. Measured 0 of 10 false negatives and 0 of 10 false positives**, recorded in the test file's own doc comment following `PIPE-105`'s convention. Twenty fixtures — one thorough and one deliberately blind pair for each of the ten operators `operatorMutations` declares.
  - **The blind fixtures are harder than `PIPE-130`'s own**, which merely discarded the result. Each of these asserts a real value that the specific mutation happens not to change — checking sign only where `+` and `-` agree, or using boundary-insensitive inputs — so a passing score cannot come from a test that checks nothing.
  - **Ground truth is not assumed:** `TestPIPE130a_GroundTruthLabelsAreNotAssumed` independently verifies a sample of the labels by hand-mutating source and running `go test` directly, outside `checkMutations` entirely. `agent_stage_mutation.go` was never touched.
  - **`MEM-015`'s shared-harness question, answered with a judgement rather than deferred:** the *methodology* is shared with `PIPE-105` — labelled fixture population, per-case failure, tallied rates, logged and documented — but the *execution harness* cannot be. `PIPE-105`'s fixtures are bare source directories read statically in microseconds; these need a real git worktree plus a real build and full test run per mutation at roughly 1.2s each, which would make `PIPE-105` pay for infrastructure it does not need.
- [x] `PIPE-131 SECURITY BLOCKER` Route acceptance runs, adversarial probes, and mutation suite runs through the mediated executor and its permission policy; all three shell out directly today, so repository code — a `TestMain`, an `init`, or any produced command — runs with the coordinator's own authority and no approval.
  - **Closed.** Every subprocess in the acceptance, adversarial-probe and mutation stages now goes through `executor.ClassifyAuthority` + `executor.ExecuteAuthorizedTool` instead of raw `exec.Command`: the acceptance build and every acceptance-example run (`agent_acceptance.go`), the named-test `go test -run` (`agent_acceptance_named_test_form.go`), `go list`, the per-command build and every hostile-input run (`agent_adversarial.go`), and `worktreeBuilds`/`suiteRejects` (`agent_stage_mutation.go`).
  - The shared call is `runMediatedVerificationCommand` in a new `agent_verification_exec.go`. It builds a `ToolRequest`/`PermissionPolicy` under a **synthetic per-call identity, never the run's real task or policy** — this is the coordinator's own verification decision, not a model's request — and mints a single-use grant for exactly that command. `internal/executor/command.go` gained a `Stdin` field on `AuthorizedToolRequest`; nothing had ever piped stdin through the mediated boundary, and acceptance examples and hostile probes need it.
  - Public signatures (`checkAcceptance`, `checkAcceptanceOracle`, `probeProducedCommands`, `checkMutations`) were deliberately left unchanged so no call site in files another lane owned had to move.
  - Verified independently: three discriminating tests, one per call site, each passing under mediation and failing without it. 15.2s.
  - **Measured cost, not estimated:** `ResolveExecutableIdentity` hashes the full `go.exe` binary on every mediated call — ~107ms isolated, against ~34µs for `LookPath`. Empirical before/after on the mutation stage by reverting and re-applying: 4.25s → 4.82s over 8 mediated calls, ~71ms/call observed. A worst-case run of 12 mutations × 2 calls adds roughly 1.3–2.5s. The unconditional hashing was left alone because it is shared by every mediated command repository-wide and narrowing it could weaken an existing check; filed as an observation rather than changed under this ticket.
- [ ] `PIPE-132 SECURITY` Bound what a probed or accepted program may reach: the probe builds and runs every main package in the repository against garbage arguments, and on a real repository those are programs the run did not write.
  - **Partial. Only the launch directory is bounded.** `validateCommandDirectory` refuses a verification command whose working directory would resolve outside the task worktree, proven by `TestPIPE132_MediatedVerificationCommandRefusesToLaunchOutsideTheWorktree` with its own unmediated counter-proof.
  - **What the ticket asks for and does not have: a bound on what a running program may *reach*.** Once the process starts, nothing restricts it. There is no firewall, socket restriction, syscall filter or filesystem jail anywhere in this codebase. The mediated boundary decides *whether a command may launch and with what environment*; it has no bearing on that process's own syscalls afterwards.
  - Stays open. Follow-up: `PIPE-132a`.
- [ ] `PIPE-133 SECURITY TEST` Add an abuse case in which repository test code attempts to read a credential, reach the network, and write outside the worktree, and prove each attempt is refused and recorded.
  - **Partial, and the test is honest about which third it proved.** `TestPIPE133_AbuseCaseCredentialNetworkAndFilesystemEscape` runs repository test code that attempts all three at once — read a credential, send it to a loopback listener, write outside the worktree — then reruns the identical fixture unmediated for discrimination.
  - **Refused: credential exfiltration.** The payload received by the listener is empty under mediation and contains the real credential without it, so the refusal discriminates. The mechanism is environment stripping (`minimalEnvironment`): the coordinator's provider key is never present in the child process, so there is nothing to send.
  - **NOT refused, and the test says so in its own output rather than asserting a containment that does not exist:**
    - `pipe131_mediation_test.go:354: recorded: network reach was NOT refused; the connection to the loopback listener succeeded under mediation`
    - `pipe131_mediation_test.go:369: recorded: the write outside the worktree was NOT refused`
  - The ticket says "prove each attempt is refused **and recorded**." Two of the three are recorded but not refused, so this stays open. Nothing in the code or the tests describes this as a sandbox; the comments in `agent_verification_exec.go` and `pipe131_mediation_test.go` state the limit directly. Follow-up: `PIPE-133a`.
...
- [x] `PIPE-132a SECURITY` Bound what a launched verification process may reach, not merely where it may launch from. `PIPE-131` routes every acceptance, probe and mutation subprocess through the mediated executor, and `PIPE-132` refuses a launch directory outside the worktree, but once the process is running nothing restricts its syscalls: it can open a socket to anywhere and write to any absolute path on the machine. `PIPE-133`'s abuse case demonstrates both, under mediation. This needs an actual containment mechanism — a job object with restricted rights on Windows, or a comparable per-process confinement — and it is the difference between "the coordinator's credential cannot be stolen" and "repository code cannot do harm."
  - **Closed, and the containment it delivers is not the one the ticket imagined.** Every mediated subprocess is now assigned to a Windows Job Object configured with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` (`internal/executor/process_windows.go`, wired into `ExecuteAuthorizedTool`), released on every exit path — success, failure, fault injection, timeout, cancellation. `golang.org/x/sys/windows` was already a dependency at v0.46.0; nothing new was added. A no-op sibling in `process_unix.go` keeps the call sites cross-platform.
  - **The real pre-existing gap it closes, which nobody had named:** `terminateProcessTree` only ever ran from the timeout and cancellation branches. A mediated command that exited **successfully** — the ordinary case — never had its descendants reaped at all, so a helper started with `Start()` rather than `Wait()` kept running unconfined, indefinitely. Closing the job now terminates every process ever assigned to it, at any depth, including one requesting `CREATE_BREAKAWAY_FROM_JOB` (denied, since neither breakaway flag is set).
  - **Discriminating, and verified by disabling and restoring rather than asserted:** `TestExecuteAuthorizedToolConfinementKillsDescendantLeftRunningAfterSuccess` has a mediated command spawn a helper that sleeps 2s then writes a marker, and returns immediately. The marker never appears; an unmediated control proves the same helper does survive and write it. With confinement temporarily disabled the test fails with `orphan-survived`. Confirmed independently here: PASS, 8.09s.
  - **Named limitation, in the code:** assignment happens immediately after `command.Start()` returns rather than via suspended-process creation, so there is a small window between OS process start and job assignment during which the child runs outside the job.
  - **Measured cost, by the same disable/restore method:** engine canary 3.91/3.96/4.13s unconfined against 4.13/4.38/4.74s confined — about 0.3–0.5s, roughly 10%. Noisy: six lanes shared the machine. Reported as real but not lab-clean.
- [ ] `PIPE-133a SECURITY TEST` Extend the `PIPE-133` abuse case to assert refusal, not merely record non-refusal, once `PIPE-132a` provides something to refuse with. The fixture and its unmediated discrimination control already exist; only the two logged `NOT refused` observations need to become assertions.
  - **The two named observations stay open, and that is the honest answer rather than a forced pass.** `PIPE-132a` delivers a Job Object, which bounds what **outlives** a command; it has no bearing on syscalls a still-running process makes. Both actions in the original abuse case — the network reach and the write outside the worktree — complete synchronously inside the fixture process before mediation is done with the command, so neither is structurally reachable by this mechanism. The two `recorded: ... NOT refused` lines in `pipe131_mediation_test.go` are **left untouched**.
  - **What was added instead, per "extend, do not rewrite": a third abuse vector in the same family that this mechanism genuinely does refuse.** `TestPIPE133a_ConfinementKillsABackgroundedExfiltrationAttemptAfterSuccess` has the fixture start a detached helper it does not wait for, betting it survives past mediation's completion. It does not. Verified the same way — disabling confinement produces `REFUSAL FAILED: the backgrounded helper reached the network under mediation after the mediated command had already finished`.
  - This proves `PIPE-132a`'s actual scope end to end through the coordinator's mediated path rather than the raw executor alone. Two fixture-design bugs surfaced en route, both Windows file-locking artifacts rather than confinement effects: re-executing the test binary as the helper collided with `go test`'s own cleanup, and building the helper under `t.TempDir()` collided with `testing.T`'s synchronous cleanup.
  - **Still true after this change:** a running mediated process can open a socket to anywhere and write to any absolute path. Closing that needs a mechanism this codebase does not have on Windows — network filtering or a filesystem jail, not a job object.
...
- [x] `PIPE-137 BLOCKER` Produce contracts from the specification rather than from the finished code: `describeContracts` reads signatures off what was written and records `derived_after: true`, so a contract cannot constrain the unit it describes. §14 requires the contract to exist first, and the atoms stage to check the implementation against it.
  - **Closed, and the discrimination proof is the point.** `describeContracts` no longer computes effects and purity by re-walking the same body the atoms stage later re-walks — under the old code `contract.effects` and `producedFunction.Pure` came from one `callsAnythingImpure` walk over one body, so they were **definitionally equal** and no fixture could make them disagree. The declared half now comes from the `//codeflux:atom` doc comment through `internal/atomdoc` (`atom_declared_contract.go`: `declaredContracts`, `declaredPurity`, `declaredEffectNames`); signature, parameter types and error cases stay derived from the type signature, which was never the vacuous part.
  - **The proof:** `TestPIPE137_ADeclaredContractCanDisagreeWithItsImplementation` — a function whose comment declares `Effects: None: pure atom.` while its body calls `fmt.Println`. The stage returns `Held=false` naming the function and what it reaches for. Control case `TestPIPE137_ADeclaredContractThatAgreesHolds` shows the same shape holding when they genuinely agree. Verified independently: both pass, 4.26s.
  - **Vacuity guard, matching PIPE-010/011:** when nothing produced carries a declaration — the shape of every existing scripted-model fixture in this repo, none of which write the schema — the stage records `Skipped`, never `Held`. It does not fabricate a contract from the code for the undeclared case; doing so would reintroduce exactly the defect.
- [x] `PIPE-138` Enforce purity where a contract declares it: the atoms gate says an atom reads nothing outside its arguments, and `checkAtoms` only counts how many happen to be pure. A unit declared pure that reaches outside itself is a gate failure, and an impure unit records what it reaches for.
  - **Closed for the purity half, and that limit is stated rather than hidden.** `checkAtoms` now fails the gate when an atom carries a declared-pure contract and its observed `Pure` is false, naming the atom and what it reaches for in `evidence["purity_violations"]`. Undeclared atoms are not judged — `TestPIPE138_AnUndeclaredImpureAtomIsNotJudged` pins that, and it is necessary: the whole existing test corpus declares nothing and would otherwise start failing this gate.
  - **Left undone:** full contract satisfaction — preconditions and postconditions actually holding — is not checked, only purity. `StageAtoms` therefore keeps its `PIPE-138` unestablished marker, which is the honest call for a partial.
- [x] `PIPE-139` Resolve effects from imports and types rather than from a fixed list of package identifiers: `callsAnythingImpure` reads `time.Duration(n)` as an effect and cannot see a call through an injected interface, so the purity figure it produces is wrong in both directions.
  - **Closed with a stated false-negative, and the wider rule was tried and rejected.** `observedEffects` resolves a selector call's package through the file's own import declarations instead of trusting the bare identifier, and narrows "this package is an effect" to a curated per-package name list. The named example is fixed — `time.Duration(n)` is excluded via `pureConversionTypes` — and a renamed import is followed correctly (`TestPIPE139_ARenamedImportIsStillResolved`).
  - **Limit, documented in the function's own comment:** a call through a value that cannot be resolved to an import — a method on a local variable, a struct field, an injected interface — is invisible. The lane tried the wider rule ("any unresolved selector call is impure") and rejected it because it flags `var b strings.Builder; b.WriteString(x)` as impure, trading the stated false-negative for a much larger false-positive across ordinary code. That is a judgment call, not a proof, and a reviewer could reasonably disagree.
- [x] `PIPE-140 BLOCKER` Compare declared effects against observed effects and fail the gate on an undeclared one. §12 makes the request-side effect the primary verification surface, and no stage currently checks that what a unit does matches what it said it would do.
  - **Closed. Declared versus observed, with no silent-pass path.** When a declared-pure entry's observed effects are non-empty, `describeContracts` fails and lists every disagreement in `evidence["disagreements"]` — the PIPE-137 fixture above is that case.
  - **What happens when effects cannot be resolved:** a function with no declaration is excluded from the comparison entirely (recorded `declared: false`, counted in `undeclared`) and is never treated as agreed; zero declared functions in a run means `Skipped`, not `Held`; a parse failure means `broke`. The only route to `Held` is at least one declared function with every declared one matching. There is no third "declared but unresolved" state because PIPE-139's resolver always returns a concrete list, possibly empty, never unknown — flagged explicitly rather than asserted by omission.
- [x] `PIPE-141` Replace `testsNaming`'s identifier sweep with call-site resolution: a unit whose name coincides with a local variable is reported verified by a test that never calls it. This is the second site of the defect `PIPE-008` repairs and is not reached by that fix.
  - **Closed, and the marker cleared.** `testsNamingInFiles` replaces the `*ast.Ident` sweep with call-site resolution — a plain call by identifier, a method or package call by selector — mirroring PIPE-008 exactly, including its deliberate strictness: a function passed as a value and invoked indirectly is not counted as named, same direction and same reasoning, documented verbatim in the new doc comment.
  - `TestPIPE141_AUnitIsNamedOnlyWhenATestCallsIt` reuses PIPE-008's own fixture shape (a coincidental local-variable/field name, a real call, a method call) against `testsNaming` specifically, since PIPE-008's fix never touched this second parse site. `StageAtomVerification`'s `PIPE-141` marker is cleared on that basis.
...
- [x] `MEM-001 BLOCKER` Open an episode when a run starts and close it when the run ends, carrying the task, the starting and ending revisions, and the run identity, so the append-only record §31 assumes exists actually exists; `OpenEpisode` and `CloseEpisode` have no production caller today.
  - `internal/coordinator/episode_lifecycle.go` wires `openRunEpisode`/`closeRunEpisode` into `AgentExecution.Run`: opened just after the pipeline ledger is created, closed through a `defer` so **every** return path closes it, not only the successful one.
  - `OpenEpisode` needs an exact fingerprint that cannot be recomputed at run time, because the inputs that produced it (`AffectedPaths`, `ToolchainBindings`) are never persisted on the task row. Migration 35 adds an immutable `task_exact_fingerprint_bindings` carry table, written once by `ForecastTask`. Opening is best-effort: a task with no binding gets **no episode** rather than a failed run or a fabricated fingerprint.
  - **Known limitation, recorded rather than hidden:** `CloseEpisode` always closes with `EpisodeOutcomeUnresolved`. `domain.NewEpisode` ties closing to the moment a terminal *user* decision is recorded, but no user decision happens inside `Run` — acceptance is a later, separate human step. Because the row freezes on close, that decision can never be written back. Filed as `MEM-001a`.
- [x] `MEM-001a DATA` Close an episode at the acceptance decision, not at the end of the agent run. `MEM-001` closes every episode as `unresolved` because `AgentExecution.Run` has no user decision to record, and `episodes_immutable_after_close` freezes the row the instant it closes — so the real outcome can never be written back. Every episode in the store therefore reports `unresolved` regardless of what happened, which makes the outcome column useless to the extractors §31 builds on top of it. Either move closure to the acceptance site or record the decision as a separate durable transition.
  - **Resolved by moving closure to the decision site.** The alternative — closing at run end and recording the decision separately — preserves the append-only shape trivially but leaves the outcome column exactly as useless as this ticket describes, permanently `unresolved` and unreadable by any extractor. The point was to make the column carry the answer.
  - `finalizeReviewDecision` in `review_mutations.go` closes the task's open episode immediately after `RecordTaskReviewDecision` succeeds, mapping Accept→`accepted`, RequestRepair/Abandon→`rejected`, Rollback→`abandoned`. Ordered so an episode-closing failure can never make a durably-recorded decision look like it did not happen.
  - `closeRunEpisode` now reads the task state at run end: a task in `awaiting-review` is **left open**, because a real decision is still pending; anything else closes as `abandoned` rather than `unresolved`, so a failed or cancelled run no longer sits open for ever. On a `GetTask` error it leaves the episode open rather than guessing — a wrong-but-present outcome is worse than one a later build can still close.
  - `closeReviewDecisionEpisode` is a no-op on an already-closed episode rather than an error, since re-closing would trip `episodes_immutable_after_close`. The trigger is untouched.
  - **No migration needed.** The outcome vocabulary and both repository methods already existed from migration 000035; `migration-check` passes unchanged.
  - **Every new invariant proven to discriminate** by removing its guard, watching the specific test fail, and restoring it: the `awaiting-review` skip, the `abandoned` outcome, the decision-site close call, and the already-closed guard. `go test -run '^$' ./...` confirms no other implementer of `reviewMutationRepositories` was broken by the interface change.
- [x] `MEM-002 BLOCKER DATA` Record each attempt's gate, normalised failure, rung, and outcome as episode action events, so the transitions every extractor reads from are facts in the store rather than strings in a chat message.
  - New table `episode_attempt_transitions` with a CHECK enforcing `normalized_failure IS NULL` exactly when the outcome is `converged`, wired into `agent_execution.go`'s `sendBack` closure and the converged break point.
  - Deliberately **not** named `episode_action_events`: that name already belongs to the M21-032 mechanism reading the `task_events` journal, and reusing it would have made two different things look like one.
  - **Gap, stated:** two branches end an attempt without going through `sendBack` — the adversarial-review-findings branch and the bottom-of-loop "tests did not pass" continuation — and are not separately recorded. Documented in the package doc.
- [x] `MEM-003 DATA` Bind the episode to the pipeline ledger's real attempt number from `PIPE-003`, so a task started twice produces two episodes and neither inherits the other's evidence.
  - **`episodes.task_id` was `UNIQUE`** — one episode per task, ever, which directly contradicts what this ticket asks for. Migration 35 rebuilds the table to `UNIQUE(task_id, attempt)` and binds the attempt to `storage.NextPipelineAttempt` through the ledger, so there is one notion of "attempt" rather than two.
  - The rebuild needed real debugging: SQLite's `ALTER TABLE RENAME TO` re-validates every trigger body left in the schema, so the child tables' triggers had to be dropped *before* the rename, not after. Established empirically with a throwaway harness rather than guessed.
...
- [ ] `MEM-004a DATA` Record the influence action and its justification for every carried item at episode close; without it eligibility never becomes influence, lineage stays unpopulated, and `ConfirmsMemoryArtifactIndependently` correctly refuses to trust anything the store holds.
  - **The path is built and tested; nothing travels on it yet, deliberately.** `internal/storage/episode_memory_influence_repository.go` adds `ListEligibleMemoryForTask`, which reads the eligible set back from `memory_retrieval_queries`/`memory_retrieval_candidates` — a *durable* consumer for `ForecastedTask.Retrieval`, since the in-process value does not survive the gap between forecast time and run time. `RecordEpisodeMemoryInfluence` writes the decision row and an `episode_memory_influence_events` row atomically, boundary-checked so a candidate must have been queried for the episode's own task.
  - It is **not** called from the agent loop, and that is the honest state rather than an omission. Doing so would mean either rendering artifact content into context — which is the Injection Surfaces milestone, `MEM-017` onward — or inventing a used/adapted/rejected verdict the loop has no mechanism to report. Recording a fabricated decision would be worse than recording none, since §31 quarantine is terminal and a cohort minted from invented influence can only be retired.
  - Closing this ticket needs `MEM-017`'s injection surface first, so the agent has a real decision to report.
- [x] `MEM-005` Record `advisory_exposure` on every episode as a write-once transition rather than a property asserted at open: an episode opens unexposed, becomes exposed the first time an advisory pattern enters, and can never be relabelled unexposed, so the permanent clean-room cohort §31 requires can be identified after the fact rather than asserted.
  - Enforced by a `BEFORE UPDATE` trigger rather than application discipline (a `CHECK` cannot compare `OLD` to `NEW`), following migration 34's precedent that a rule worth stating is worth putting in the schema.
  - **Proven load-bearing:** the trigger was temporarily deleted, the schema regenerated, and the discriminating test confirmed failing (`raw relabel to unexposed error = <nil>, want ErrConstraint`) before it was restored.
  - **No call site exists to wire it to**, because nothing in the repository injects an advisory pattern into a run yet — that is `MEM-017`/`MEM-018`. The mechanism is real and ready; it is not yet exercised by production.
...
- [ ] `MEM-012 BLOCKER` Mint a candidate at the strongest tier that can express the observation — workspace fact, mechanical rule, regression case, routing evidence, advisory pattern — and refuse a weaker tier without a recorded reason the stronger one could not carry it.
  - **The decision layer is built and tested; it is not yet in any path. These stay open.** `internal/coordinator/memory_candidacy_funnel.go` is pure and storage-independent, and **nothing in production calls it** — verified by symbol search: `DetermineMemoryCandidacyTier`, `RefuseInadmissibleMemoryCandidacySource` and `SelectStrongestExpressibleMemoryCandidacyTier` have zero non-test references. A gate nothing calls does not gate anything, which is the same standard applied to `PIPE-132`/`PIPE-133` above and the same shape as the extractors that sat uncalled from M21 until `MEM-006`.
  - **Why it cannot be wired yet, established by reading rather than assumed:** `domain.MemoryArtifactKind` has eight kinds covering only two of §31's five candidacy tiers — workspace fact (RepositoryFact / ReviewedCommand / FileToTestMapping / RepositoryConvention) and regression case (AcceptedRegressionCase). **Mechanical rule, routing evidence and advisory pattern have no domain payload at all.** The funnel returns an abstract tier because there is nothing concrete to return for three of five. Adding those content types is `MEM-015` and `MEM-017`+, not these tickets.
  - **What did already exist:** `internal/domain/memory.go` carries a full maturity/authority state machine that already excludes `agent_self_report` from *promoting* an artifact to an authority-bearing state. What was missing is candidacy itself — `domain.NewMemoryArtifactRevision` mints a `Candidate` from any content regardless of source, and `UpsertExtractedMemoryFact` checks self-report only at the evidence-recording step, not at admission. So all four tickets were genuine gaps, not partials.

  - `MemoryCandidacyTier` (five tiers, strongest first), `MemoryCandidacyObservation` with per-tier expressibility fields, `SelectStrongestExpressibleMemoryCandidacyTier`, and `DetermineMemoryCandidacyTier`, which refuses a caller-requested weaker tier with `ErrMemoryCandidacyWeakerTierUnjustified` unless a non-empty justification is recorded. Logic done; **not called**.
- [ ] `MEM-013 BLOCKER` Refuse candidacy from `agent_self_report`, from a run whose produced work was not attributed to changed declarations, and from unapproved repository text, so the sources §31 names as inadmissible cannot enter through an extractor rather than through retrieval.
  - `RefuseInadmissibleMemoryCandidacySource` refuses `EvidenceSourceKindAgentSelfReport`, unattributed produced work, and unapproved repository text outright, at any tier, **before** any tier logic runs — which is what closes the "enters through an extractor" back door the ticket names. Logic done; **not called**, so the back door is still open in fact.
  - **No approval-granting path for repository text was built**, deliberately, following `MEM-009`'s precedent: inventing one would open a prompt-injection route into every later run.
- [ ] `MEM-013a BLOCKER` Refuse a first-attempt success only for the tiers that require something to have been contested — advisory pattern, regression case, and routing evidence about where a run stalled — and admit it for the deterministic tier, since a command that ran and exited zero is an observation of what executed rather than a conclusion about what was hard. Refusing it outright would forbid `MEM-006` through `MEM-008` on the runs that go well, which are most of them.
  - `requiresContestedFirstAttempt` refuses `Contested: false` for advisory pattern, regression case and routing evidence only; workspace fact and mechanical rule are admitted regardless, so `MEM-006` through `MEM-008` on runs that go well — most of them — are unaffected. This is the distinction the ticket asks for and it is implemented as written. **Not called.**
- [ ] `MEM-012a BLOCKER` Call the candidacy funnel from the extraction path. `MEM-012`, `MEM-013` and `MEM-013a` implement tier selection and inadmissible-source refusal as a pure decision layer with **no production caller**, so nothing is currently refused in fact. Wiring waits on domain content types for mechanical rule, routing evidence and advisory pattern (`MEM-015`, `MEM-017`+), because three of the five tiers have nothing concrete to mint. Until this lands, treat the three tickets above as designed and tested rather than enforced.
...
- [x] `MEM-016 TEST` Prove the funnel refuses to mint an advisory pattern for an observation a mechanical rule expresses, using a fixture whose failure is a swallowed error, which the anti-patterns stage already checks.
  - **Closed, and it is a real discrimination rather than a self-consistent assertion.** `TestMemoryCandidacyRefusesAdvisoryPatternAMechanicalRuleAlreadyExpresses` runs the repository's **real** `findAntiPatterns` (from `agent_stage_antipatterns.go`, unmodified) over a fixture with a genuine swallowed error, confirms the real mechanical check fires on it, and then proves the funnel refuses an advisory-pattern request for that same observation and resolves to `MemoryCandidacyTierMechanicalRule`.
  - Discrimination was established by temporarily bypassing all three refusal points to simulate no-funnel behaviour: five tests fail as expected, including this one, and pass again once restored.
...
- [x] `REPO-018 BLOCKER` Clear the `staticcheck` findings. These block `REPO-017` and every Go commit.
  - **The recorded count and diagnosis were both wrong.** By 2026-08-02 the count was 51, not 44, and **19 of them were not defects at all**. `staticcheck` ran only under the host target, so every symbol in `web/` whose callers sit behind `//go:build js && wasm` was reported unused while being live, wired frontend code. Two lanes reached that diagnosis independently before any code was deleted; acting on the ticket as written would have broken the shipped frontend.
  - The same blind spot ran the other way: the wasm-tagged files were **never analysed at all**, hiding six real findings including a swallowed error (`SA4006`).
  - Fixed in two parts. The gate repair is `CL-20260802-035` (`runStaticcheckPerBuildTarget`: host pass over non-browser packages, browser-host pass with `U1000` disabled, js/wasm pass with tests disabled). The source clearance covers the 32 genuine findings plus the six newly visible ones.
  - The failing `test-fast` package named in the original wording was resolved separately by `REPO-032`.
  - `CL-20260802-036`. All three passes report zero findings: host over every non-browser package, browser-host with `U1000` disabled, js/wasm with tests disabled. 32 unreachable declarations removed; two control characters escaped; one loop, one bool comparison and one nil check simplified; five deprecated fixed waits in the browser suite replaced with waits on the real condition.
  - **One finding was a product defect, not a style issue.** `web/client/timeline_mounted.go` assigned the older-page fetch error and never read it. On the page-merge failure path the feed returned with `LoadingOlder` still true and no `SafeError`, and nothing else clears that flag — so "load older messages" wedged permanently on a spinner with no error shown until the user switched threads. That defect was invisible to the gate for as long as the browser packages went unanalysed.
  - Verified: `go build ./...`, `GOOS=js GOARCH=wasm go build ./web/client/...`, `gofmt`, and `go test` over `./web/client/`, `./web/frontend/...`, `./internal/executor/`, `./cmd/codeflux-dev/` all green.
  - **Capabilities removed rather than wired** — each needs its own governing task, filed below as `REPO-036` through `REPO-038`.
- [x] `REPO-036 UX` Wire graph search in the browser client, or delete the `SearchGraph` RPC.
  - **Wired.** Deleting a working, tested server capability was the worse option. `web/frontend/graphcanvas/search.go` adds the search box and its loading, empty and populated states, built from `primitives.Button` so keyboard and ARIA come from the primitive rather than being hand-rolled. `searchGraphResource` mirrors `expandGraphResource`'s shape and reuses `decodeGraphResource` unchanged, since the RPC returns a full slice view.
  - `useMountedGraph` gains debounced-by-supersession search with a stale-response guard. `selectSearchResult` centres and selects a match already in the loaded slice, and **reports plainly when the match is not loaded** — the search covers the server's whole graph while the view holds a bounded slice, and silently doing nothing there would look like a broken control.
  - Reused the dead `LinkedChatNodeID` field as `FocusNodeID` rather than adding a second select-and-centre mechanism beside it. It had zero call sites anywhere and is exactly what a search result needs.
  - **Browser-verified against a purpose-built fixture, because no live graph is reachable** (`REPO-041`): `web/frontend/graphcanvas/searchfixture` mounts the renderer over an in-memory three-node graph with no coordinator or database. Driven with real typing and keystrokes — loading state appeared, two results returned, Tab reached a result button, Enter activated it, the node reported `aria-pressed="true"`, and the viewport centred on it. A zero-match query was driven too. Screenshots opened and judged; the box sitting flush to the viewport edge is an artifact of testing a component with no page chrome, not a product defect. `searchGraphResource` was removed by `REPO-018` because it had no caller: its sibling transport functions (`expandGraphResource`, `loadGraphNodeResource`, `compareGraphResources`) are all mounted in `graph_mounted_wasm.go` and this one never was, and no search control exists in the graph surface. The backend `SearchGraph` RPC in `internal/coordinator/graph_query.go` is real and implemented, so the server answers a question the interface cannot ask.
- [ ] `REPO-037` Decide whether adversarial review should check the worktree is clean. `worktreeIsClean` was removed by `REPO-018` as uncalled. It was built for verifying a build survives an optimisation rewrite, and `checkSimplification` performs no rewrite (`PIPE-010`), so it never had a caller. If `PIPE-010` ever performs a rewrite, this check has to come back with it.
- [x] `REPO-038` Remove `bootstrapEnvelope.SelectedSessionID` from the bootstrap contract, or consume it.
  - **Resolved as "keep it" — my framing of this ticket was wrong.** I wrote that the server populates a field nothing reads. It is dead in `web/client`, confirmed: unmarshalled and never read in `restorationContext` or `routeAccessKey`. But it is read outside, by three test files that assert on it directly.
  - `internal/frontendtest/production_console_test.go` **requires** it present once a repository auto-opens, with its own comment: "Without the session the client starts no stream at all… it simply has nothing to connect to." `internal/coordinator/application_test.go` requires it **absent** in an earlier bootstrap state. `internal/frontendserver/handler_test.go` asserts on it in three places.
  - So the field's presence and absence is a live, actively-checked proxy for "has a session been established", even though the client derives its session from the thread instead. Removing it from the client struct alone would create an inconsistency for no benefit; the JSON field is harmless to keep.
  - Anyone revisiting this should do it together with those three test files, and should replace the proxy with a direct assertion rather than deleting the signal.
- [x] `REPO-024` Re-check the vendored `frontend-design` skill by hand periodically.
  - **No drift as of 2026-08-02.** `.claude/skills/frontend-design/SKILL.md` and `LICENSE.txt` compared against the cached marketplace copy: SHA-256 matches on both, same file set, `diff` reports nothing. Nothing was updated because there was nothing to update, and `AGENTS.md` forbids downloading external prompt content mid-task in any case.
- [x] `REPO-037` Decide whether adversarial review should check the worktree is clean.
  - **Decided: leave it removed, and do not restore speculatively.** `checkSimplification` still performs no rewrite — its body records `"rewritten": false` with a comment reaffirming `PIPE-010`. `PIPE-010`'s scope was to record the stage as skipped, not to add a rewrite, so closing it did not create a caller. No ticket exists yet that implements the atom-optimization rewrite, and `PIPE-017` notes it is gated behind a mutation score that also does not exist.
  - Whichever future ticket implements that rewrite must re-add `worktreeIsClean` alongside it, in `internal/coordinator` beside `checkSimplification`. Restoring it now would put back an uncalled function to satisfy a caller nobody has scheduled. `REPO-018` removed the browser client's `startAuthenticatedSession` path as dead; the field is still populated by the server and read by nothing. Session bootstrap ships through the thread-driven path in `timeline_mounted.go`, which derives the session from `thread.SessionID()`. A server field nobody reads is a contract that claims a selection mechanism the client does not use.
- [ ] `REPO-035` Give `go vet` the same per-build-target treatment `REPO-018` gave `staticcheck`, so the browser packages' wasm-only source is vetted at all. It is currently not: `lint` runs `go vet ./...` under the host target only, and a naive js/wasm pass fails to compile because browser *test* files import the coordinator and through it SQLite. `go vet` has no `-tests=false`, so this needs either a package-level split that excludes browser test packages or a different vet invocation. Not urgent — `staticcheck` under js/wasm now covers the same source for its own check set — but the gap is real and currently unrecorded anywhere else.
- [x] `REPO-019` Land the fixed dev-server port `127.0.0.1:47311`. Written and passing its own checks; `CL-20260802-018` is drafted and uncommitted because the pre-commit hook refuses the tree for other lanes' failures.
  - **It landed, but not the way the ticket expected.** `cmd/codeflux/start.go` declares `DefaultListenAddress = "127.0.0.1:47311"` at `HEAD`, introduced by the checkpoint commit `92f1eed` rather than by its own feature commit. `CL-20260802-018` was already tracked in `CHANGELOG`; its `Commit:` field is now back-filled to `92f1eed`.
  - **Known defect in the binding, recorded rather than hidden:** `git log --grep="Change-Log: CL-20260802-018"` returns nothing, because the checkpoint carried the change under its own message and the trailer was never written. `CHANGELOG` states the trailer is the authoritative binding and the hash a convenience; for this one entry that is inverted, and the hash is the only binding there is. This is precisely the class of damage `REPO-020` exists to prevent, and it is the concrete case that ticket should be tested against.
- [x] `REPO-020` Enforce that a commit carries both ledger trailers and that the referenced entries exist. The most-violated rules in this repository are the unenforced ones.
  - **The check lives in a new `.githooks/commit-msg`, not in `lint`, and the ticket's own wording was wrong about where it belongs.** `lint` runs from `pre-commit`, which fires *before* a commit message exists — `git commit -m` has not supplied one and an editor has not opened. Reading `.git/COMMIT_EDITMSG` was considered and rejected: it persists after every commit ever made, so a standalone or CI `lint` would validate a stale message or, on a fresh clone, none. `commit-msg` is git's dedicated hook, fires once per attempt, and receives the proposed message. A consequence worth stating: `codeflux-dev lint` structurally cannot fail on this rule, which is what keeps CI honest.
  - The hook validates trailer shape and then resolves each identifier against the **staged** `CHANGELOG`/`DEVLOG` (`git show :CHANGELOG`), so an entry added in the same commit counts and a dangling reference does not. It does not re-invoke `lint`, which would double every commit's cost.
  - `cmd/codeflux-dev/commit_msg_hook_test.go` drives the real hook file through actual `git commit` calls in a temporary repository — missing trailers, malformed trailer, well-formed trailer with no ledger entry, and the valid case. It also reproduces `CL-20260802-018` (swept into checkpoint `92f1eed` with no trailer at all, `git log --grep` returning nothing) as a fixture, proving the hook would have refused the one commit whose ledger binding this repository actually lost.
- [x] `REPO-020a` Decide what to do about `CL-20260802-018`, whose commit carries no `Change-Log:` trailer because it was swept into checkpoint `92f1eed`. `REPO-020`'s hook prevents a recurrence but cannot repair it: adding the trailer means rewriting a released commit. The hash is back-filled in `CHANGELOG`, so the binding exists in one direction only. Either accept that permanently and say so in `CHANGELOG`'s preamble, or record the exception where a reader of `git log --grep` will find it.
  - **Both, because one place is not enough.** The `CHANGELOG` preamble's Commit-binding section now records the standing exception, so a reader learning the general rule learns immediately that one entry breaks it. `CL-20260802-018`'s own `Compatibility-or-migration` field repeats it, so a reader who arrives at the entry directly — which is the likelier path — does not have to know the preamble exists.
  - The commit recording this decision names `CL-20260802-018` in plain prose rather than only as a trailer. An un-anchored `git log --grep="CL-20260802-018"` therefore resolves to something that points at `92f1eed`, which the exact `Change-Log:` trailer grep alone can never do. That is the closest thing to a repair available without rewriting a released commit.
- [x] `REPO-021` Prevent ledger identifier collisions between lanes. Sequential `CL-` and `DL-` numbers on a shared branch collide; either derive them collision-free or make `lint` reject a duplicate.
  - **Measured 2026-08-02: it collided six times, not once.** `CHANGELOG` carries `CL-20260802-028` twice (lines 302, 327). `DEVLOG` carries `DL-20260802-004` (2106, 2262), `-012` (1456, 1604), `-014` (1392, 1508), `-015` (1290, 1352) and `-038` (380, 411) twice each. Reproduce with `grep '^Change-ID: CL-' CHANGELOG | sort | uniq -d`.
  - This constrains the fix: all six are **released entries**, which `AGENTS.md` forbids rewriting or renumbering, so a rule that simply rejects duplicates would make `lint` red on the current tree and re-block every commit. The enforceable shape is a ratchet — refuse any *new* duplicate, with the existing six declared as an exact legacy set that must not grow. A count-based tolerance would let a new collision hide behind a removed one.
  - **Done.** `cmd/codeflux-dev/ledger_identifiers.go`, registered in `runRepositoryChecks`. Refuses a malformed identifier and any duplicate, exempting the six historical collisions by a **content-addressed** allowlist — SHA-256 of each duplicate entry's full text rather than a bare identifier — so a later lane cannot delete one of the six and slip an unrelated new collision in under the same ID.
  - Parsing is anchored to the `Entries` marker. Both ledgers open with a field-name template listing bare `Change-ID:` / `Dev-Log:` labels with no values, and an unanchored scan reads those as a malformed entry; the first implementation did exactly that and turned `lint` red on the repository's own file header. There is a fixture for it, because a line-number skip would start passing over a real defect the day the preamble grows.
  - Tests: clean fixture passes, the preamble-template regression passes, duplicate `Change-ID` and `Dev-Log` both fire, malformed IDs fire, a substitute duplicate reusing a legacy ID with *different* content still fires, and the real ledgers pass.
- [ ] `REPO-022` Add `test-browser` to the main gate once its CI runtime is measured, and add its context to ruleset `20243842` in the same change. Until then `REPO-008` cannot be verified in CI.
  - **Measured 2026-08-02, and the gating step failed: the suite is RED, so nothing was wired.** A required check that blocks `main` cannot be a failing one, which is what this ticket's own precondition says.
  - Runtime, measured against a real run rather than estimated: WASM client build 36.7s cold / 4.2s warm (module cache already populated, so a blank runner pays download on top); `test-browser` itself 582.7s (9m43s), exit 1. Most of that is dozens of accumulated 3s and 30s timeouts, not work — a passing run should be far shorter, and that number is not yet known.
  - **Root cause of the red, and it is a fixture-drift defect, not flakiness:** `internal/frontendtest/matrix.go` still expects `RouteHeadings["/tasks"] = "Implement the Codeflux frontend shell"`. `production_console_test.go`'s `NothingOnScreenIsInvented` documents that exact string, with `$0.42`, `Milestone` and `§27A`, as **invented fixture content that must no longer appear** — the interface used to boot from a fake demo before it was wired to real data. `matrix.go` was never updated when the fixture was removed. Filed as `REPO-039`.
  - **One independent real regression** surfaced underneath it: `keyboard-full-shell-forward-reverse-visible-focus` fails on its own at tab stop 26 ("Collapse task details sidebar") — reachable but not reported focused. Filed as `REPO-040`.
  - Platform recommendation when it is wired: one job on **Ubuntu 24.04 AMD64**, matching the existing race job's runner. Playwright's Chromium and its `apt` system dependencies are well supported there; Windows ARM64 has no official Playwright Chromium build, and running a browser suite across all four matrix runners is a cost multiplier nobody asked for.
  - The ruleset half stays blocked and **must not be attempted by an agent**: ruleset `20243842` currently requires seven contexts, and a check context cannot be added until a job exists to publish one. The required string is the new job's `name:` field verbatim, matching the existing pattern.
  - Order of unblocking: `REPO-039` and `REPO-040`, then a clean re-run for a real pass/fail and warm timing, then the CI job, then the server-side ruleset call, then record that call under `REPO-016`.
  - **Update after `REPO-039` and `REPO-040` landed: still red, ~550–565s across three consecutive runs.** Both of those defects are genuinely fixed and verified in isolation; the remaining ~13 failures are a third, larger problem — the mounted matrix asserts against thread, graph, and timeline fixture state that no supported path creates on a clean database. Filed as `REPO-041`, which now blocks this ticket.
- [x] `REPO-039 BLOCKER TEST` Update `internal/frontendtest/matrix.go`'s `RouteHeadings` to the headings the product actually renders.
  - Fixed; `direct-route-heading` no longer fails on `/tasks` or `/memory` at any viewport. The assertion was not weakened to a vague substring — it still pins real rendered content, which is the only reason the check is worth having.
  - It was **not** the only thing making `test-browser` red. See `REPO-041`. `/tasks` still expects `"Implement the Codeflux frontend shell"`, which `production_console_test.go` explicitly lists as removed fixture invention. `/memory` fails the same way. This makes `direct-route-heading` fail on every viewport and cascades into `wide-graph-visible`, every `graph-canvas-*` check and every `m17-*` mounted check, which then sit through 30s timeouts each. It is the single reason `test-browser` is red, and therefore the reason `REPO-008`'s driven-browser rule cannot be enforced in CI at all. Blocks `REPO-022`.
- [x] `REPO-040 BLOCKER TEST` Fix the keyboard focus regression at tab stop 26.
  - **A genuine product accessibility defect, and it was hiding behind a test-visibility gap.** In `web/frontend/composer/component.go` the "Options" and "Task controls" trigger buttons ignored the `disabled` parameter that every sibling control — task-class select, attachments, override fields — respects. They stayed natively `disabled=false` while their containing `<section data-component="composer">` correctly reported `aria-disabled="true"`.
  - The gap is the interesting part: Playwright's `keyboardTabStops()` honours ARIA-disabled ancestry, so the check never saw those stops; a real browser's Tab key does not, so a sighted keyboard user lands on "Options" mid-sequence. Every later stop index desynced from the check's snapshot, and "Collapse task details sidebar" — the next real stop — was never pressed. **An accessibility check that models ARIA more faithfully than the browser does will miss exactly the defects that matter to a keyboard user.**
  - Established by raw single-Tab reproduction (from "Open task review", landing on `#composer-options` every time, independent of snapshot timing), not inferred. Fixed with `disabled ||` on both buttons, matching the pattern every other composer control already used.
- [ ] `REPO-041 BLOCKER TEST` Give the mounted browser matrix a way to reach a populated thread, or the `graph-canvas-*` and `m17-*` checks can never pass. `test-browser` remains red after `REPO-039` and `REPO-040` with ~13 failures, all requiring a durably-open thread carrying a real ~10-node task graph and real timeline cards. `loadInteractionThread` always navigates to bare `/tasks`, which is deliberately a shell-preview route that adopts no thread — unlike `/memory` and `/graphs`, which adopt the coordinator's `OpenThreadID`. Nothing in a freshly-migrated database can satisfy those checks, and the one purpose-named seed database on disk (`.artifacts/local-frontend-m17-final/codeflux.sqlite3`) fails migration-22 checksum verification against current `HEAD`.
  - **Not closed. The matrix now runs against a real populated thread, and that surfaced more failures than it fixed — which is the honest result.** `codeflux-dev seed success` under `CODEFLUX_DEV_SEED_SERVE=1` drives one real requirement through a real `coordinator.Application` over real SQLite, reaching `awaiting-review` in ~11s, and serves it. `internal/testharness/mounted_thread.go`'s `StartMountedThread` uses only exported symbols. The descriptive default output is untouched, so `TestAUDIT029_SeedListsAndDescribesTheClosedScenarioSet` still passes (confirmed, 4/4).
  - **Verbatim result:** `status=failed total=260 passed=235 failed=25`. Not the ~13 this ticket predicted, and **not the same failures** — seeding a real thread reached checks that had never executed at all. The node classes were read back out of the database rather than guessed: 11 real nodes across all 6 wired classes, three uniquely addressable by accessible name and three duplicate-label pairs that are real rather than defects. Node IDs are captured dynamically from `data-node-id` because they are fresh ULIDs per seed.
  - **The largest finding is an architecture fact the ticket predates: the graph canvas is no longer inline on `/tasks` at wide viewport.** `TaskWorkspaceShell` moved it to the observation rail, and `RailGraphSummary`'s comment says "the graph itself has a page" — a dedicated `/graphs` route. That is why `standard-graph-visible` passes and `wide-graph-visible` fails. Four checks assert `[data-component="graph-canvas"]` exists inline on `/tasks` at wide and are **unsatisfiable by design**: `wide-graph-visible`, `graph-canvas-mounted-high-dpi-accessible-overlay`, `graph-canvas-viewport-selection-and-deliberate-inspection`, `graph-canvas-responsive-visible-bounds`. They need to navigate to `/graphs`. Left failing rather than relabelled. Follow-up: `REPO-041a`.
  - **A genuine state bug surfaced, unrelated to seeding:** `graph-selection-survives-narrow-tabs-and-viewport-churn` selects successfully, then after cycling compact→standard→wide→minimum the page reverts to "No task yet" while the transcript stays populated. Task selection is lost on resize. Screenshot captured. Follow-up: `REPO-041b`.
  - **A mistake the lane reported against itself:** `runRouteContractChecks` in `runner.go` calls `_ = selectDurableThreadRow(...)` and swallows the error, so five narrow-viewport composer/tab failures have no established cause. Reported as unresolved rather than guessed.
  - Remaining failures need content a single-thread seed does not produce: distinct timeline card kinds, a thread literally titled "Untitled thread", a "Home" nav control. **No assertion was weakened to reach green, and `test-browser` is not reported as green.**
  - **Defect in what was built, stated by the lane rather than left implicit:** `StartMountedThread`'s `defer Shutdown` only runs on a clean exit. Killed hard — which happened under memory pressure — the coordinator and its worker survive holding `.artifacts/seed/mounted/coordinator.sqlite3` and `codeflux-worker.exe`, and the next `seed` fails with `unlinkat ... Access is denied`. Reproduced independently twice while running the command by hand. Follow-up: `REPO-041c`.
  - **The implication is worth stating: these checks appear never to have passed against a clean database.** The mounted matrix has been asserting against fixture state that no supported path creates, which is the same class of defect as `REPO-039`'s stale headings and a larger instance of it.
  - Fixing it means one of: route-adoption changes in `web/client/main.go`, a deterministic seeding path in `internal/coordinator` (`codeflux-dev seed` already exists and could grow a mounted-frontend scenario), or a committed, migration-current fixture database. The third is the least invasive and the most likely to rot; the second matches what `AUDIT-029` already built.
  - **Investigated 2026-08-02. The diagnosis above is incomplete and the seeding framing is partly wrong.** Three findings, each traced rather than inferred:
  - **Route 2 is wrong on the merits, and there is a cheaper fix already in the file.** `web/client/main.go` adopts the coordinator's `SelectedThreadID` for `/graphs` and `/memory` only, *by design* — those routes describe the run that is open globally, while bare `/tasks` is deliberately a thread **picker**. Not a gap. And `interaction_checks.go` already contains `selectDurableSessionForComposer`, which lands on `/tasks`, opens the rail when narrow, and clicks a `thread-row` — that is how the composer checks already reach a real thread with no product change. `loadInteractionThread` should do the same instead of assuming bare `/tasks` auto-opens one.
  - **The checks are pinned to fixture content the product deliberately destroyed.** `graph_canvas_checks.go` and `interaction_checks.go` (written 2026-07-31, commit `aeda85d`) assert node ids `requirements`/`responsive`/`implementation` and labels "Shell requirements - Complete", "Responsive layout - Active", "GWC workspace - Active". Commit `964e09d` the very next day (`CL-20260801-012`) deleted those exact strings from `reference_chrome.go` as the invented ten-node demo, **at the user's explicit request to stop using seeded data**. Reseeding that content would resurrect precisely the anti-pattern the product just removed. This is `REPO-039`'s defect class at larger scale and needs the same treatment — rewrite the expectations to real produced content, do not manufacture a scenario shaped to satisfy dead strings.
  - **One assertion is unsatisfiable by any run the coordinator can currently perform**, which is a product gap rather than a seeding gap. Filed as `REPO-044`.
  - Measured baseline for this ticket: `test-browser` exit 1, **657.7s**, **15** failing checks. **Now 12**, after `REPO-046`, `REPO-047` and `REPO-048` landed — 248 of 260 checks pass, and every remaining failure is this ticket's own thread-and-graph data gap. Three of them — `visible-descendant-bounds`, `shortcut-dialog-restores-invoker-focus`, `m17-first-run-single-functional-scroll-owner` — were not in the earlier set and do not fit the thread/graph story. Filed as `REPO-045`.
- [x] `REPO-044 BLOCKER` Emit the graph node classes the projector already implements, or stop asserting them. `graph-canvas-mounted-high-dpi-accessible-overlay` requires `nodeCount == 10` and at least 4 distinct shapes. `internal/graph/projector.go` implements all seven `NodeClass` values, but `agentGraphRecorder` — the only projector caller wired into a live task run — emits exactly three: `ProjectionRequirementAccepted` once, `ProjectionOperationObserved` per edit, `ProjectionEffectObserved` per command. `ProjectionPlanDeclared`, `ProjectionValidationObserved`, `ProjectionArtifactObserved`, `ProjectionApprovalBoundary` and `ProjectionRecoveryObserved` have **no production caller anywhere**; their only caller is a unit test driving the projector directly, and `declarePlan`'s own comment records that it deliberately does not project a plan region yet.
  - **Resolved by wiring the emitters.** `docs/plan.md` states Program mode's shape as requirements to plan regions to atoms and branches to external effects to validation obligations to generated artifacts. A projector implementing seven classes behind a recorder emitting three is not the "optional and derived" simplification the boundaries allow -- it is a graph silently dropping most of its stated shape. Deleting the unemitted classes would have discarded a built, tested projector to match an under-wired caller.
  - **Three classes wired, each at the point its fact becomes true:** `PlanRegion` inside `declarePlan`, after `RecordPlanRevision` has committed -- projecting before that commit would be a plan region declared before a plan exists, the dishonest case exactly. `Obligation` inside the tool executor's test branch, where the validation's own outcome becomes known, projected *in addition to* the existing Effect node: the Effect says a command ran, the Obligation says whether the requirement it exists to check was satisfied. `ArtifactResult` after `RecordProducedArtifact` returns, because drawing an artifact before the store confirms it would show a result nothing backs.
  - **A real run now produces six distinct shapes, up from three.**
  - **Two classes deliberately not wired, with their sites named.** `ApprovalBoundary`'s durable fact is granted in `advanceToReady` *before a task starts*, so no per-run recorder exists yet; wiring it honestly means either creating the graph earlier or teaching the recorder to read back an already-committed approval, which is an architecture decision, not a ticket detail. `RecoveryObserved` belongs to a ten-file subsystem that resumes a task whose original recorder is long gone, and needs a resume path the recorder does not have. `BranchMatchMerge` stays unemitted; it is the class both of those use.
  - **Discrimination proven by disabling each emitter in turn** and confirming that specific class vanishes and only that class -- including one case where disabling the plan region made `recordFileEdit` fail outright on an edge referencing an unprojected node. A third test proves the negative: `declarePlan` with no durable revision projects **nothing**, an absent node rather than a lying one.
  - So no real task reaches four distinct node shapes, and the check was written past what the product does. Two honest resolutions: wire the missing emitters so the graph shows plan, validation, artifact and approval structure a run genuinely has — which is the graph's stated purpose — or drop the assertion to the three classes `agentGraphRecorder` can produce, which is an admission that the graph currently explains less than it claims.
  - Blocks `REPO-041` and therefore `REPO-022`.
- [x] `REPO-045 TEST` Triage three browser checks that began failing during the 2026-08-02 session. `visible-descendant-bounds` (elements overflowing the standard viewport), `shortcut-dialog-restores-invoker-focus`, and `m17-first-run-single-functional-scroll-owner` (on `/first-run`) were not in the previously measured failure set and do not fit `REPO-041`'s thread-and-graph story. The measuring binary was built after migration `000036`, `PipelineStageService`, and changes to `agent_execution.go`, `episode_lifecycle.go`, `review_mutations.go` and `agent_plan_contract.go` landed, so they are plausibly collateral from that work rather than pre-existing. **Establish whether they are regressions before assuming they are not** — a check that started failing during a session of heavy change is the one worth chasing first. Screenshots are under `.artifacts/repo041-tb-run/`.
  - **All three are pre-existing, established rather than assumed.** None of the frontend paths they exercise was touched anywhere in `92f1eed..dev`, and the session's backend work has **zero frontend wiring** — `grep -r PipelineStageService web/` returns nothing. Reproduced live against current `dev` HEAD with a real headless browser rather than resting on that reasoning alone.
  - **`visible-descendant-bounds`** — the graph legend's "Memory" entry overflows the 1280px viewport by ~19px (right edge 1299.04). `graphLegend` declares `Overflow.Hidden`, `MinWidth(Zero)` and ellipsis, but its flex ancestor chain never constrains its width, so it never clips. Last touched in `81bfd1c`, five hours **before** this session's checkpoint. Filed as `REPO-046`.
  - **`shortcut-dialog-restores-invoker-focus`** — **passes standalone.** In the suite it runs immediately after `runKeyboardTraversalCheck` on the same shared page, and that check is independently known-broken, plausibly leaving focus state dirty. An order-dependent flake, not a regression. Filed as `REPO-047` — a suite where one check's failure corrupts the next is a harness defect in its own right.
  - **`m17-first-run-single-functional-scroll-owner`** — a real product defect, and the most serious of the three. Filed as `REPO-048`.
- [x] `REPO-048 BLOCKER` The first-run page never leaves its loading skeleton. `web/client/main.go`'s `coordinatorStore()` dispatches `FirstRunChanged{FirstRun: FirstRunView{State: DataLoading}}` exactly once at boot, and `FirstRunChanged` is **never dispatched again anywhere in the codebase** — verified as a single call site. So `routeStateContent` never leaves the loading state, `/first-run` renders placeholder bars for all five setup cards for ever, and the fifth card's "Open tasks" call to action never mounts.
  - **Fixed, and first-run genuinely needs no remote data — established, not assumed.** `firstRunLayout` renders entirely static copy: step number, title, status pill, body, action. `FirstRunView.CurrentStep` and `.Completed` exist and are read by nothing. So the loading state was never waiting on a fetch somebody meant to write; it was waiting on nothing.
  - Dispatches `DataReady` rather than `DataReadyEmpty`, in the same per-render block as the other panes' answers. The surface has real content on every visit, so "empty" would have been the wrong word for a reader.
  - Verified live: rebuilt the WASM client, ran it against an isolated server, confirmed all five region states report `ready`, screenshotted the fully rendered cards, **clicked "Open tasks" and confirmed navigation to `/tasks`** — the journey a new user could not previously complete.
  - **This is the plan's first prototype journey, and a new user cannot complete it.** The page a person sees first is permanently stuck on skeletons. It predates the 2026-08-02 session entirely and was surfaced by `REPO-045`'s triage.
  - To close: dispatch a second `FirstRunChanged` — `DataReady` or `DataEmpty` — once the client has established that first-run needs no remote data. Related to `AUDIT-016a`, which records that the same journey cannot complete over the API either.
- [x] `REPO-046 UX` Stop the graph legend overflowing the standard viewport. Its container declares `Overflow.Hidden`, `MinWidth(Zero)` and ellipsis, but nothing in the flex ancestor chain constrains its width, so the "Memory" entry spills ~19px past 1280px and `visible-descendant-bounds` fails on `/tasks` at the standard viewport. Pre-existing since `81bfd1c`.
  - **Fixed, and my diagnosis in this ticket was wrong.** I wrote that nothing in the flex *ancestor chain* constrained the width. The container was already correctly bound with `MinWidth(Zero)` and `Overflow.Hidden`. The real defect was one level in: the five per-entry spans had no `MinWidth(Zero)` of their own, so their content-based automatic minimum size stopped them shrinking.
  - The distinction matters and is worth keeping: **`overflow: hidden` clips paint, not layout geometry.** The last entry's box still measured past the container and the viewport even though nobody would have seen it painted there — which is exactly why a bounds check caught what looking at the screen did not.
  - Each entry span now carries `MinWidth(Zero)` and `Overflow.Hidden`, and each label additionally `WhiteSpace.NoWrap` with an ellipsis so it truncates rather than merely clipping.
  - Verified by a live DOM scan: **0 of 380 descendants exceed the 1280px viewport**, down from 2.
- [x] `REPO-047 TEST` Stop one browser check's failure corrupting the next. `shortcut-dialog-restores-invoker-focus` passes standalone and fails in the suite, because it runs immediately after `runKeyboardTraversalCheck` on the same shared page and that check is independently broken, leaving focus state dirty. Either reset focus state between interaction checks or give them separate pages. A suite where a failure cascades into unrelated checks reports more red than it has defects, which makes triage cost more than it should — this ticket exists because that cascade cost a lane real time.
  - **Fixed at the shared entry point.** `loadInteractionThread` is the single function every interaction check calls, and the suite reuses one Playwright page — so a check that failed earlier could leave a real element focused on the way into the next one. It now blurs the active element before handing the page on, so every check starts from the same neutral state regardless of what the last one left behind.
  - This is why `shortcut-dialog-restores-invoker-focus` passed standalone and failed in the suite: it was inheriting focus from the independently-broken keyboard traversal check ahead of it.
  - Blocks `REPO-022`, which was previously thought to be blocked only on `REPO-039`. `keyboard-full-shell-forward-reverse-visible-focus` reports "Collapse task details sidebar" as reachable in the tab order but not focused when tabbed to. This one is independent of `REPO-039` and fails on its own. `AGENTS.md` requires walking the whole keyboard path with visible focus at each stop, so this is a product defect and not a test-expectation defect. Blocks `REPO-022`.
- [x] `REPO-043 SECURITY` Upgrade `google.golang.org/grpc` from `v1.81.0` to `v1.82.1`. Dependabot reports one open HIGH alert against the default branch: "gRPC-Go: xDS RBAC and HTTP/2 Vulnerabilities", vulnerable range `< 1.82.1`. Surfaced by GitHub on the 2026-08-02 push; it predates this session's work and no change here touched `go.mod`.
  - Not cosmetic for this repository: `internal/coordinator` serves the worker protocol over gRPC and `internal/frontendserver` carries the browser bridge, so the HTTP/2 half of the advisory is on a live path even though it is loopback-bound. The xDS half is not — nothing here uses xDS.
  - `PIPE-033` will want a dependency-audit stage that catches this class automatically rather than by a push-time notice.
  - **Done.** `go get google.golang.org/grpc@v1.82.1` + `go mod tidy`. One transitive move: `google.golang.org/genproto/googleapis/rpc` to `v0.0.0-20260414002931-afd174a4e478`. `golang.org/x/net` and `google.golang.org/protobuf` did not move. **No API breakage** — no call site changed anywhere in the coordinator, frontend server or transport.
  - Confirmed by grep that no xDS usage exists here, so the xDS RBAC half was never reachable and the HTTP/2 half was the real exposure.
  - Verified: `go build ./...`, `go vet ./...`, `gofmt` clean; `go test ./internal/frontendserver/` (full package, including the websocket bridge, same-origin and loopback security tests) and the transport/session/`TestAUDIT008` set all green; `lint` exit 0.
- [x] `REPO-027a` Make CI actually enforce the coverage ratchet.
  - `.github/workflows/ci.yml`'s coverage step now reads `coverageFloorPercent` out of `cmd/codeflux-dev/coverage_floor.go` at run time and passes it to `--min-coverage`, rather than restating the number in YAML — which would have been the third copy of a value that already needed a test to keep two copies in step.
  - A value must be passed: `resolveCoverageThreshold` reads only the flag or `CODEFLUX_MIN_COVERAGE` and leaves the floor **disabled** when neither is set, so a bare invocation asserts nothing.
  - **Fails in the safe direction:** the step `throw`s when the constant cannot be found, so a formatting change to the constant breaks CI loudly instead of silently passing an empty floor and disabling the gate.
  - Known brittleness worth watching: it is a regex over Go source, so a rewrite like `const coverageFloorPercent float64 = 60.0` would stop matching. It fails loudly when that happens, which is the acceptable version of that risk; a `codeflux-dev` subcommand printing the floor would remove it entirely.
...
- [x] `REPO-026 BLOCKER` Establish the actual coverage baseline.
  - **Measured 2026-08-02. Default (untagged) total: 62.6%. With `-tags=integration`: 63.6%.** Both suites passed in full; profiles and per-package breakdowns under `.artifacts/coverage/`.
  - The two figures differ by about one point repo-wide, which is the more useful finding than either number: the packages `REPO-030` re-tagged move a great deal in isolation (`internal/testharness` 58.1%→89.3%, `internal/testfixtures` 34.9%→86.5%) but are small against the module, so the aggregate barely shifts. `internal/storage` and `internal/retrieval` did not move at all.
  - Per-package range is roughly 12%–100%, most packages in the 65–90% band.
  - This is the figure the 80% floor in `AGENTS.md` was always being compared against, and it settles `REPO-028`. The only profile on disk is a stale partial from 2026-07-30 reporting 26.9% of one package, which is not a repository figure.
  - **Corrected by `REPO-033`:** this item previously read "It is unmeasured: `test-fast` fails, so no complete profile exists." That is no longer true. `REPO-032` identified the failing package as `internal/coordinator`'s ladder test and gated it behind `CODEFLUX_LADDER`; `go test ./... -count=1` on `49061c4` was 92 packages ok, 0 failures. The baseline is measurable now, and the stated dependency on `REPO-018` was about `lint`, not about the suite.
- [x] `REPO-027` Teach `codeflux-dev test-coverage` a `--min-coverage` threshold so CI can enforce the same floor the hook does. Today the command writes a profile and asserts nothing, so the floor exists only on developer machines and only for people who ran `git config core.hooksPath .githooks`.
  - Added, with `.githooks/pre-push` now delegating to it rather than keeping a second implementation. Precedence: `--min-coverage` beats `CODEFLUX_MIN_COVERAGE`; neither set means no floor, which preserves today's behaviour for the bare CI invocation.
  - Both non-negotiable behaviours from `AGENTS.md` are preserved and tested: a **failing suite is refused rather than measured** (the command returns the test's own exit code before any threshold logic runs, and prints that coverage was not measured), and a push carrying no Go change still skips the measurement — that check stays in the hook, since a command has no notion of a push.
  - Tests run against an isolated throwaway module with deterministic 50% coverage, so below-floor, at-floor, env-only, flag-beats-env, and unenforced-by-default are each exercised on a real profile. Four functions were hand-mutated in turn and the corresponding test confirmed failing before being reverted.
- [ ] `REPO-027a` Make CI actually enforce the coverage ratchet. `.github/workflows/ci.yml` still invokes `test-coverage` bare, so it writes a profile and asserts nothing — the floor is enforced by the pre-push hook and by nothing else. Add `--min-coverage` to that invocation. Left undone by `REPO-027` because `.github/` was outside that lane's ownership.
- [x] `REPO-028` Decide whether the floor should be a ratchet rather than a constant. A fixed 80% is unreachable from the current baseline and will be bypassed until it is met; a ratchet that refuses any decrease is enforceable from day one and converges on the same place.
  - **Decided: ratchet.** With the measured baseline at 62.6%, the ticket's own argument holds — 80% is not reachable today and a floor people must bypass teaches them to bypass floors.
  - `coverageFloorPercent` in `cmd/codeflux-dev/coverage_floor.go`, set to 60.0: a few points below the baseline as margin for the Windows-versus-Ubuntu-CI platform difference and run-to-run noise, with that reasoning recorded at the constant rather than left as a mystery number.
  - The decrease protection is the part that makes it a ratchet rather than a lower constant: `TestCoverageFloorNeverDecreases` reads the value as of the last commit that touched the file (`git show <prior>:cmd/codeflux-dev/coverage_floor.go`) and fails if the working tree's is lower. A second test keeps `.githooks/pre-push`'s shell default in sync with the Go constant so the two cannot drift apart silently.
- [x] `REPO-029` Scope the pre-commit hook to unit tests for the changed packages instead of the whole repository.
  - `CL-20260802-028`. `.githooks/pre-commit` derives the package list from the staged Go files and runs `go test` untagged against those only.
  - Untagged means nothing behind the `integration` build tag is built here.
- [x] `REPO-030` Put the heavy tests behind the `integration` build tag so the suites the commands already declare become real.
  - **Proven by count, not by adjective:** `go test ./... -list '.*'` selects 2554 tests untagged and 2587 with `-tags=integration`. The difference of 33 is exactly the 5 + 12 + 16 test functions moved behind the tag, so the two commands finally select different sets.
  - Six files tagged, each measured first: `internal/coordinator/engine_assembly_test.go` (50.1s), `engine_refinement_test.go` (30.1s), `engine_pipeline_ledger_test.go` (35.5s), `internal/testharness/harness_test.go` (5.2s) and `layers_test.go` (7.2s), `internal/testfixtures/harness_test.go` (6.9s). Before tagging, every unexported symbol was cross-referenced against every other test file in the package to prove nothing untagged would stop compiling.
  - Wall clock: `internal/testharness` 11.6s → 0.6s, `internal/testfixtures` 7.4s → 0.4s, `internal/coordinator` 128.9s → 77.4s on a warm build cache.
  - **Two of the ticket's own assumptions did not survive measurement.** `internal/retrieval` was named as heavy and is not: 5.5s for the whole package, heaviest file 2.0s. It was left untagged. And `internal/storage` could not be tagged safely: its test files share fixtures almost all-to-all (`openTestRepositories` is used by 42 of ~80 files, and `testWorkspaceID` is defined in the single heaviest file but needed by three fast ones), so tagging any heavy file breaks the untagged build for files that call its helpers.
  - Also left untagged with reasons: `engine_end_to_end_test.go`, which defines the fixture scaffolding eight other files use; and `pipeline_ledger_integrity_test.go`, which mixes one 27s test with five sub-second ones, and a build tag is file-granular.
- [x] `REPO-030a` Extract `internal/storage`'s shared test fixtures into an always-compiled helper file so its heavy tests can be tagged. `REPO-030` established that this is the only blocker: the package's ~80 test files share helpers almost all-to-all, so no heavy file can be tagged without breaking the untagged build for fast files that call it. `internal/storage` is ~135s and the largest remaining cost in `test-fast`. This is a mechanical refactor across dozens of files and deserves its own task rather than being attempted blind inside a tagging ticket.
  - **Done. `internal/storage` untagged drops from 238.8s to 178.2s**, about a quarter, and the package keeps 293 of its 343 tests under `test-fast` — 72 of 83 files — so no unit-test collapse.
  - Selection counts, the evidence this ticket asks for: **2635 untagged against 2718 tagged, a delta of 83** — 50 from this change (the exact sum of test functions in the 11 tagged files) plus `REPO-030`'s pre-existing 33.
  - Ten shared helpers moved verbatim into an untagged `shared_fixtures_test.go`: `testWorkspaceID`, `setTaskRunStates`, `openMigrationTestDatabase`, `validContextManifestInput`, the validation-run fixtures, the graph fixtures, and the session-event fixtures. Each was checked for transitive dependencies on other heavy-file-only symbols before moving; none had any.
  - **The dependency map caught its own bug**, which is why the result is trustworthy. A first pass mis-split two-token `type X` / `const X` / `var X` declarations, and the tell was twelve unrelated symbols all reporting an identical suspicious 19-file usage list. Redone with unambiguous delimiters: 168 declarations across 83 files, 79 with real cross-file consumers.
  - Tagging threshold set at a measured natural break — the heaviest untagged file is 8.9s and the lightest tagged is 11.0s — rather than at a round number.
  - No file in the repository carries `//go:build integration` in either syntax, so `codeflux-dev test-integration` — `go test -tags=integration ./...` — currently selects exactly the same tests as `test-fast`. The distinction exists in the command graph and nowhere in the code.
...
- [x] `REPO-031 BLOCKER` Make the atom-comment opening rule read the sentence rather than the first line. `validateAtomDeclaration` in `cmd/codeflux-dev/atoms.go` takes `firstDescriptiveCommentLine` and requires that one physical line to both start with the identifier and contain `.`, `!`, or `?`. An opening sentence that wraps -- which a long identifier plus a descriptive clause does at the repository's comment width -- fails, and the message says the comment "must begin with X" when it already does. This blocks `codeflux-dev lint`, and therefore every lane's pre-commit, on `internal/storage/spend_attribution_repository.go:145`, whose comment reads "AttributeSpend slices one window's recorded provider spend by flow phase, / stage, and model." and is correct Go style.
  ...
  - `CL-20260802-034`. `openingAtomSentence` in `cmd/codeflux-dev/atoms.go` joins comment lines up to the first sentence terminator, and stops at a blank line, the schema header, the atom marker, or a name exception so a genuinely missing opening sentence is not rescued by a full stop later in the prose. The message now names the text it found.
  - **Second finding, same file, reachable only once the first was fixed:** `AttributeSpend` was then rejected with `first word "Attribute" is not a recognized concrete action verb`. It is one; `concreteActionVerbs` was incomplete and now contains it. An `//codeflux:atom-name-exception` was deliberately not used — an exception is a reviewed record that a name breaks a rule, and this name does not.
  - Both fixes ride in one commit because each alone still leaves `lint` red, and a red `lint` cannot pass the pre-commit hook.
  - Verified: new fixture `wrapped-opening.go` passes; new fixture `unterminated-opening.go` fails naming what it found, so the relaxed rule still discriminates; `identifier-mismatched` and `keyword-stuffed` still fail. `internal/storage/spend_attribution_repository.go` is unchanged. `codeflux-dev lint` clears the repository check and now stops at staticcheck (`REPO-018`).
...
- [x] `REPO-042` Stop the pre-commit hook building `testdata` directories as packages.
  - `CL-20260802-054`. The hook derives the packages to test from the staged Go files' parent directories, so a changed lint fixture named `cmd/codeflux-dev/testdata/atomdocs` and `go test` built it. The Go tool ignores `testdata` precisely so fixtures can be invalid on purpose; naming one explicitly overrides that.
  - Those seven fixtures all declare `ValidateTaskBudgetBeforeModelRequest` by design, so the directory cannot compile and **no commit touching a fixture could ever pass the hook.** Found the moment `REPO-031` — which adds two fixtures — tried to commit.
  - Filtered every `testdata` path segment rather than special-casing `atomdocs`, and guarded the empty case: a commit staging only fixtures would otherwise have run `go test` with no arguments, silently testing the current directory instead.
- [x] `REPO-033` Correct the two records that assumed the ladder always runs. `REPO-026` blocks the coverage baseline on "`test-fast` fails, so no complete profile exists", which is no longer true and makes the baseline measurable now. `REPO-030` says a commit touching `internal/coordinator` "still pays the nine minutes"; it pays them only when `CODEFLUX_LADDER` is set, so the tagging argument now rests on `internal/storage`, `internal/testharness`, `internal/testfixtures`, and `internal/retrieval` alone.
  - Both records corrected in place, each carrying a note naming this ticket and the superseded wording, per the rule that a released record is corrected by a later entry rather than rewritten silently. `REPO-026`'s stale premise mattered most: it declared the coverage baseline unmeasurable and blocked itself on a suite failure that `REPO-032` had already fixed.
- [x] `REPO-034 TEST` Decide whether a provider-backed test may exist without an explicit opt-in. **Decided: no.** Design constraint found while implementing it — the rule must match the repository's provider-key *resolution helper*, not raw `os.Getenv` on key-shaped names. A first attempt matched the environment reads and flagged `internal/worker/environment_test.go`, which sets a synthetic `OPENAI_API_KEY` into a child process and reads it back **to assert it is empty**, proving the worker environment strips credentials. Requiring an opt-in gate there would have made a credential-leak security test skip by default — a worse outcome than the spend it guards against. A test that reaches for a *real* key is the only case that costs money, and calling the helper is the signal for that.
...
- [ ] `LAD-001` Let the decomposition name the layout. `filesNamedIn` (`internal/coordinator/agent_narration.go`) falls back to exactly `cmd/generated/main.go` and its test whenever the requirement names no path, so the number of packages a run can produce is a constant. Ladder rung 16 asks for "one package exports Mean and Max, and the command imports that package" and got a plan holding one directory; the run then satisfied 31 stages with none failing and delivered one package, because every gate judges the plan and the plan was already wrong. Rungs 16, 17, 18 and every later rung carrying `minPackages` are blocked on this.
  - The layout has to come from the decomposition, which already reads the requirement and names the behaviours on `gpt-5.6-sol:high` (`atomic-instructions`). That is a schema change to the decomposition response plus a validator: the files it names must be inside the module, must include a command, and must carry a test beside each source file, or `agentPlanSteps` cannot build steps that the gates asking for tests can bind to.
  - **Do not infer the layout from prose.** `storage.AnalyzeTaskRequirement` extracts `ExplicitSymbols` only from backtick-quoted code spans, and these requirements write `Mean` and `Max` as ordinary words, so a symbol-based rule never fires. A capitalised-word heuristic was tried and reverted: it misfires on sentence-initial words and would make the plan depend on English punctuation.
  - `decomposition-coverage` currently reports "every file the request named is covered by a step, and no step writes a file the request did not ask for" and is satisfied vacuously when the request named nothing. Once the decomposition names files, that gate is where a layout contradicting the request should be refused -- before any code is written, rather than at the end by a ladder assertion the pipeline itself cannot see.

- [ ] `LAD-002` Make the atom registry actually fill. Every ladder run this session ends with `atom-registration` and `molecule-registration` failing advisorily — "0 of N produced atom(s) reached the registry, so no later task can find this work" — and the ladder's own summary confirms the consequence: "The recall stage searched 0 earlier artifact(s) in total and found 0 function(s) that already existed". The compounding-effort thesis is unstarted rather than unproven: no run can reuse what no run ever registered.
  - `TestPIPE048_ChangedContractMintsADifferentAtomIdentity` fails and is the cheapest way in — it registers two functions with genuinely different contracts into one project and finds one revision, not two. Confirmed failing at `5577f21`, so it predates the 2026-08-04 pipeline work.
  - The stored row in that failure has every documentation field empty (`{None:false Reason: Text: Items:[]}` for all nineteen) while `SourceCommentHash` and `NormalizedInputHash` are populated and `ValidationStatus:admitted`. An admitted revision whose document is blank is either a parse that silently produced nothing or a redaction that blanked it; establish which before changing behaviour.
  - Two separate symptoms share this ticket and may not share a cause: a declaration with no `//codeflux:atom` block at all (`SkipReason` "no //codeflux:atom schema-v1 documentation comment is attached"), and one whose block exists and is rejected. The first is a gate question — nothing asks a run for the schema block in terms it can satisfy — and the second is a storage question.
...
- [ ] `LAD-004` The RPC catalog and the product descriptor disagree about how many RPCs exist. `TestRPCContractCatalogMatchesProductDescriptor` reports "catalog has 60 RPCs; descriptor has 61" and has done since at least `5577f21`, so one of the two has been wrong for a while and nothing downstream noticed. Establish which is authoritative before adding or removing anything: a catalog that undercounts hides an RPC from whatever reads it, and a descriptor that overcounts documents one that does not exist.
