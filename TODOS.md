...
- [ ] `PIPE-130a TEST` Measure the mutation score's false-positive and false-negative rates across many fixtures. `PIPE-130` proved the score **discriminates** — a blind-test fixture scores 0% and a thorough one 100% on identical source — which establishes the mechanism works, but not how often it is wrong. Rates need repeated trials over a fixture population, and until they exist the threshold is a number nobody has calibrated. Related to `PIPE-105`, which asks the same question of the critic's finders.
