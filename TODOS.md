...
- [ ] `REPO-036 UX` Wire graph search in the browser client, or delete the `SearchGraph` RPC. `searchGraphResource` was removed by `REPO-018` because it had no caller: its sibling transport functions (`expandGraphResource`, `loadGraphNodeResource`, `compareGraphResources`) are all mounted in `graph_mounted_wasm.go` and this one never was, and no search control exists in the graph surface. The backend `SearchGraph` RPC in `internal/coordinator/graph_query.go` is real and implemented, so the server answers a question the interface cannot ask.
